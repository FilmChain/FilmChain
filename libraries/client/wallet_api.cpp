...
#include <fcc/utilities/words.hpp>
...
namespace mcc { namespace client { namespace detail {
                                         ...
                                         const string& new_passphrase_verify )
{
    ...
    if( new_passphrase.size() < MCC_WALLET_MIN_PASSWORD_LENGTH )
        FC_CAPTURE_AND_THROW( password_too_short );

    if( brain_key.size() && brain_key.size() < MCC_WALLET_MIN_BRAINKEY_LENGTH )
...
string detail::client_impl::wallet_get_account_public_address(const string& account_name) const
{
...
vector<mcc::wallet::account_address_data> detail::client_impl::wallet_list_my_addresses() const
{
...
void read_keys( const fc::variant& vo, vector<private_key_type>& keys, const string& password )
{
    try
    {
      ...
      const auto key = mcc::utilities::wif_to_key( wif_key );
    ...
    catch( const mcc::wallet::invalid_password& e )
    {
        throw;
    }
    catch( ... )
...
bool detail::client_impl::wallet_check_passphrase(const std::string& passphrase)
{
...
{
	const asset amount = _chain_db->to_ugly_asset(amount_to_transfer, asset_symbol);
	auto entry = _wallet->transfer_asset_to_address(amount,
		from_account_name,
		to_address,
		memo_message,
		strategy,true);
...
{ try {
    ...
    _wallet->write_latest_builder( *new_builder, builder_path); // always write to "latest"
    return *new_builder;
...
{ try {
    ...
    const share_type max_supply = MCC_BLOCKCHAIN_MAX_SHARES;
...
{ try {
    ...
    auto account_record = _chain_db->get_account_record(issuer_account);
...
transaction_builder detail::client_impl::wallet_uia_issue(
...
{ try {
    ...
    //return record;
        ...
        auto builder = _wallet->create_transaction_builder();
        builder->uia_issue_or_collect_fees(true, amount, recipient, memo_message, true);
        builder->finalize(false);
        auto record = builder->sign();
        bool sign_finished = builder->is_signed();
        if (sign_finished)
        {
            _wallet->cache_transaction(builder->transaction_record);
            network_broadcast_transaction(builder->transaction_record.trx);
        }
        if(builder_path!="")
            _wallet->write_latest_builder(*builder, builder_path);
        return *builder;
} FC_CAPTURE_AND_RETHROW( (asset_amount)(asset_symbol)(recipient) ) }
...
transaction_builder detail::client_impl::wallet_uia_update_active_flags(
        const string& paying_account,
        const string& asset_symbol,
...
{ try {
    auto builder = _wallet->create_transaction_builder();
    builder->uia_update_permission_or_flag( paying_account, asset_symbol, flag, enable_instead_of_disable, false, true );
    builder->finalize(false);
    auto record= builder->sign();
    bool sign_finished=builder->is_signed(); 
    if (sign_finished)
...
transaction_builder detail::client_impl::wallet_update_asset_authority(
    const string& account_name,
    const string& asset_symbol,
    const vector<string>& vec_addr,
    uint32_t min_addr_num,
    const bool add_or_remove,
    const std::string& builder_path
    )
{

    auto result_pair = _wallet->uia_update_authority(account_name, asset_symbol, vec_addr, min_addr_num, add_or_remove==true?asset_authority_action::add: asset_authority_action::remove, true);
    
    if (result_pair.first == true)
    {
        _wallet->cache_transaction(result_pair.second.transaction_record);
        network_broadcast_transaction(result_pair.second.transaction_record.trx);
    }
    _wallet->write_latest_builder(result_pair.second, builder_path);
    return result_pair.second;
}

transaction_builder detail::client_impl::wallet_set_asset_authority(const std::string& account_name, const std::string& asset_symbol, const std::vector<std::string>& vec_addr, uint32_t min_addr_num, const std::string& builder_path)
{
    auto result_pair = _wallet->uia_update_authority(account_name, asset_symbol, vec_addr, min_addr_num, asset_authority_action::brand_new, true);
    if (result_pair.first == true)
    {
...
{ try {
  ...
  for (auto& prettytrx : history) {
      ...
      int i = 0;
      for (auto& prettyledgerentry : prettytrx.ledger_entries) {
          if (prettyledgerentry.amount.amount == 0&& prettyledgerentry.memo=="")
          {
              //取出为0的ledger
              to_remove.push(i);
              invalid_set.insert(i);  
          }
          else
          {
              int j = 0;
              for (auto& ledg : prettytrx.ledger_entries)
              {
                  if (i == j)
                      break;
                  if (invalid_set.find(j) != invalid_set.end())
                  {
                      j++;
                      continue;
                  }
                  if ((ledg.to_account == prettyledgerentry.to_account)
                      && (ledg.memo == prettyledgerentry.memo)
                      && (ledg.from_account == prettyledgerentry.from_account)
                      && (ledg.amount.asset_id == ledg.amount.asset_id))
                  {
                      ledg.amount.amount+= prettyledgerentry.amount.amount;
                      to_remove.push(i);
                      invalid_set.insert(i);
                  }
                  j++;
      ...
      auto ledger_begn=prettytrx.ledger_entries.begin();
      while (to_remove.size() > 0)
      {
          int remov=to_remove.top();
          to_remove.pop();
          prettytrx.ledger_entries.erase(ledger_begn + remov);
      }
  }
...
vector<mcc::wallet::escrow_summary> client_impl::wallet_escrow_summary( const string& account_name ) const
{ try {
...
{
  FC_ASSERT(0,"interface disabled");
...
{
  if( fee_symbol.empty() )
     return _wallet->get_transaction_fee( _chain_db->get_asset_id( MCC_BLOCKCHAIN_SYMBOL ) );
...
{
    FC_ASSERT(0, "operation disabled");
...
mcc::wallet::transaction_builder client_impl::wallet_uia_issue_to_asset(const std::string& account, const std::string& issue_symbol, const std::string& target_symbol, const std::string& issue_amount,const std::string& builder_path)
{
    auto acct = _wallet->get_account(account);
    address from_addr = acct.owner_address();
    asset ugly_asset = _chain_db->to_ugly_asset(issue_amount, issue_symbol);
    auto builder = _wallet->create_transaction_builder();
    auto fee = _wallet->get_transaction_fee();
    builder->withdraw_transaction_fee(account,fee);
    auto target_asset_rec=_chain_db->get_asset_record(target_symbol);
    if (!target_asset_rec.valid())
        FC_CAPTURE_AND_THROW(unknown_asset_symbol,(target_symbol));
    builder->uia_issue_to_asset(ugly_asset, target_asset_rec->id);
    builder->sign();
    auto record = builder->sign();

    auto entry = ledger_entry();
    entry.from_account = acct.active_key();
    entry.amount = ugly_asset;
    entry.memo = "issue to asset " + target_symbol;

    builder->transaction_record.ledger_entries.push_back(entry);
    builder->transaction_record.fee = fee;
    bool signe_finished = builder->is_signed();
...
mcc::wallet::wallet_transaction_record client_impl::wallet_transfer_to_asset(const std::string& account, const std::string& transfer_amount, const std::string& transfer_symbol, const std::string& target_symbol)
{
    const asset amount = _chain_db->to_ugly_asset(transfer_amount, transfer_symbol);
    auto res= _wallet->transfer_asset_to_asset(amount,account,target_symbol);
    _wallet->cache_transaction(res);
    network_broadcast_transaction(res.trx);
    return res;

}
} } } // namespace mcc::client::detail
