...
#include <fcc/blockchain/account_operations.hpp>
...
{
   if( first > 0 ) --first;
   FC_ASSERT( first < MCC_BLOCKCHAIN_NUM_DELEGATES );
   FC_ASSERT( first + count <= MCC_BLOCKCHAIN_NUM_DELEGATES );
   vector<account_id_type> all_delegate_ids = _chain_db->get_active_delegates();
   FC_ASSERT( all_delegate_ids.size() == MCC_BLOCKCHAIN_NUM_DELEGATES );
...
vector<string> client_impl::blockchain_list_missing_block_delegates( uint32_t block_num )
{
   ...
   timestamp += MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
...
oaccount_record detail::client_impl::blockchain_get_account( const string& account )const
{
   try
   {
      ...
      else if( account.substr( 0, string( MCC_ADDRESS_PREFIX ).size() ) == MCC_ADDRESS_PREFIX ) {
...
variant_object client_impl::blockchain_get_info()const
{
   ...
   info["name"]                                 = MCC_BLOCKCHAIN_NAME;
   info["symbol"]                               = MCC_BLOCKCHAIN_SYMBOL;
   info["address_prefix"]                       = MCC_ADDRESS_PREFIX;
   info["db_version"]                           = MCC_BLOCKCHAIN_DATABASE_VERSION;
   info["genesis_timestamp"]                    = _chain_db->get_genesis_timestamp();

   info["block_interval"]                       = MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
   info["delegate_num"]                         = MCC_BLOCKCHAIN_NUM_DELEGATES;
   ...
   info["name_size_max"]                        = MCC_BLOCKCHAIN_MAX_NAME_SIZE;
   info["memo_size_max"]                        = MCC_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE;

   info["symbol_size_max"]                      = MCC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE;
   info["symbol_size_min"]                      = MCC_BLOCKCHAIN_MIN_SYMBOL_SIZE;
   info["asset_shares_max"]                     = MCC_BLOCKCHAIN_MAX_SHARES;
   info["short_symbol_asset_reg_fee"]           = _chain_db->get_asset_registration_fee( MCC_BLOCKCHAIN_MIN_SYMBOL_SIZE );
   info["long_symbol_asset_reg_fee"]            = _chain_db->get_asset_registration_fee( MCC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE );

   info["statistics_enabled"]                   = _chain_db->get_statistics_enabled();

   info["relay_fee"]                            = _chain_db->get_relay_fee();
   info["max_pending_queue_size"]               = MCC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
   info["max_trx_per_second"]                   = MCC_BLOCKCHAIN_MAX_TRX_PER_SECOND;
...
{ try {
   ...
   return asset( _chain_db->calculate_demcc( include_interest ).at( asset_id ), asset_id );
...
{ try {
   ...
   const share_type pay_per_block = MCC_MAX_DELEGATE_PAY_PER_BLOCK * average_delegate_pay_rate / 100;
...
market_history_stat client_impl::blockchain_market_stat(const std::string& quote_symbol,
                                                        const std::string& base_symbol,
                                                        const fc::time_point& start_time,
                                                        const fc::microseconds& duration,
                                                        const market_history_key::time_granularity_enum& granularity) const
{
    auto history_points = _chain_db->get_market_price_history( _chain_db->get_asset_id(quote_symbol),
                                                               _chain_db->get_asset_id(base_symbol),
                                                               start_time, duration, granularity );
    market_history_stat stat;
    stat.stat_base_volume = stat.stat_quote_volume = 0;
    price highest, lowest;

    for (auto i = 0; i < history_points.size(); ++i)
    {
        if (i == 0)
        {
            stat.stat_opening = history_points[i].opening_price;
            stat.stat_closing = history_points[i].closing_price;

            highest = _chain_db->to_ugly_price(history_points[i].highest_bid, base_symbol, quote_symbol, true);
            lowest = _chain_db->to_ugly_price(history_points[i].lowest_ask, base_symbol, quote_symbol, true);

        }
        else
        {
            auto bid = _chain_db->to_ugly_price(history_points[i].highest_bid, base_symbol, quote_symbol, true);
            auto ask = _chain_db->to_ugly_price(history_points[i].lowest_ask, base_symbol, quote_symbol, true);
            auto high = std::max(bid, ask);
            auto low = std::min(bid, ask);
        ...
        stat.stat_base_volume += history_points[i].base_volume;
        stat.stat_quote_volume += history_points[i].quote_volume;
    }
    
    stat.stat_highest = _chain_db->to_pretty_price(highest, false);
    stat.stat_lowest  = _chain_db->to_pretty_price(lowest, false);
...
bool client_impl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
{
    ...
    catch (...)
    {
        try {
           ...
           // NB LHS of == operator is mcc::blockchain::public_key_type and RHS is fc::ecc::public_key,
...
vector<mcc::blockchain::string_status_record> client_impl::blockchain_list_markets()const
{
   const vector<pair<asset_id_type, asset_id_type>> pairs = _chain_db->get_market_pairs();

   vector<mcc::blockchain::string_status_record> statuses;
...
vector<mcc::blockchain::market_transaction> client_impl::blockchain_list_market_transactions( uint32_t block_num )const
{
   return _chain_db->get_market_transactions( block_num );
}

mcc::blockchain::string_status_record client_impl::blockchain_market_status( const std::string& quote,
...
mcc::blockchain::asset client_impl::blockchain_unclaimed_genesis() const
{
