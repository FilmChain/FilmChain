...
#define UPDATE_CONFIG(name, value) delegate_config config = _delegate_config; \
                                   ...
                                   fc::json::save_to_file( _delegate_config,_data_dir/"delegate_config.json");
...
{ try {
    ...
    _chain_db->set_relay_fee(fee);
    _config.min_relay_fee = fee;
    fc::json::save_to_file(_config, _data_dir / "config.json");
