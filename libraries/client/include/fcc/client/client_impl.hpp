...
#include <fcc/client/notifier.hpp>
#include <fcc/net/chain_server.hpp>
#include <fcc/net/upnp.hpp>
...
class client_impl : public mcc::net::node_delegate,
      public mcc::api::common_api
{
public:
   ...
   client_impl(mcc::client::client* self, const std::string& user_agent) :
      _self(self),
      ...
      _blockchain_synopsis_size_limit((unsigned)(log2(MCC_BLOCKCHAIN_BLOCKS_PER_YEAR * 20)))
   {
      try
      {
   ...
   // @{
   virtual bool has_item(const mcc::net::item_id& id) override;
   virtual bool handle_message(const mcc::net::message&, bool sync_mode) override;
   virtual std::vector<mcc::net::item_hash_t> get_item_ids(uint32_t item_type,
                                                           const vector<mcc::net::item_hash_t>& blockchain_synopsis,
                                                           uint32_t& remaining_item_count,
                                                           uint32_t limit = 2000) override;
   virtual mcc::net::message get_item(const mcc::net::item_id& id) override;
   virtual fc::sha256 get_chain_id() const override
   {
      FC_ASSERT( _chain_db != nullptr );
      return _chain_db->get_chain_id();
   }
   virtual std::vector<mcc::net::item_hash_t> get_blockchain_synopsis(uint32_t item_type,
   ...
   virtual uint32_t get_block_number(const mcc::net::item_hash_t& block_id) override;
   virtual fc::time_point_sec get_block_time(const mcc::net::item_hash_t& block_id) override;
   virtual fc::time_point_sec get_blockchain_now() override;
   virtual mcc::net::item_hash_t get_head_block_id() const override;
   ...
   mcc::client::client*                                    _self;
   std::string                                             _user_agent;
   mcc::cli::cli*                                          _cli = nullptr;
   ...
   mcc::rpc::rpc_server_ptr                                _rpc_server = nullptr;
   std::unique_ptr<mcc::net::chain_server>                 _chain_server = nullptr;
   std::unique_ptr<mcc::net::upnp_service>                 _upnp_service = nullptr;
   ...
   mcc::net::node_ptr                                      _p2p_node = nullptr;
   bool                                                    _simulated_network = false;

   mcc_gntp_notifier_ptr                                   _notifier;
   ...
   // include all of the method overrides generated by the mcc_api_generator
   // this file just contains a bunch of lines that look like:
   // virtual void some_method(const string& some_arg) override;
#include <fcc/rpc_stubs/common_api_overrides.ipp> //include auto-generated RPC API declarations
};
