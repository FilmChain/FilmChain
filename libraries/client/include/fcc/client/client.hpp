...
#include <fcc/client/seed_nodes.hpp>
#include <fcc/net/node.hpp>
#include <fcc/rpc/rpc_client_api.hpp>
#include <fcc/rpc_stubs/common_api_client.hpp>
...
namespace mcc { namespace client {
    ...
    struct rpc_server_config
    {
      ...
      fc::path         htdocs;
      map<string,mcc::api::permissions> users;
    ...
    struct config
    {
        ...
        uint16_t            maximum_number_of_connections = MCC_NET_DEFAULT_MAX_CONNECTIONS;
        bool                use_upnp = true;
        ...
        share_type          min_relay_fee = MCC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
        string              wallet_callback_url;

        share_type          light_network_fee = MCC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
        share_type          light_relay_fee = MCC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
        ...
        optional<string>    growl_mccshares_client_identifier;
    };
     ...
     *
     */
    class client : public mcc::rpc_stubs::common_api_client,
                   public std::enable_shared_from_this<client>
    {
       public:
         ...
         mcc::rpc::rpc_server_ptr   get_rpc_server()const;
         mcc::net::node_ptr         get_node()const;
         fc::path                   get_data_dir()const;

         // returns true if the client is connected to the network
         bool                is_connected() const;
         mcc::net::node_id_t get_node_id() const;
         ...
         bool handle_message(const mcc::net::message&, bool sync_mode);
         void sync_status(uint32_t item_type, uint32_t item_count);

       protected:
         virtual mcc::api::common_api* get_impl() const override;
...
extern const std::string MCC_MESSAGE_MAGIC;


FC_REFLECT(mcc::client::client_notification, (timestamp)(message)(signature) )
FC_REFLECT( mcc::client::rpc_server_config, (enable)(enable_cache)(rpc_user)(rpc_password)(rpc_endpoint)(httpd_endpoint)(websocket_endpoint)
            (encrypted_rpc_endpoint)(encrypted_rpc_wif_key)(htdocs)(users)(whitelist) )
FC_REFLECT( mcc::client::chain_server_config, (enabled)(listen_port) )
FC_REFLECT( mcc::client::config,
        (logging)
        (ignore_console)
        (client_debug_name)
        (rpc)
        ...
        (growl_password)
        (growl_mccshares_client_identifier)
        (rpc)
    )

FC_API( mcc::api::common_api,
        (about)
        (get_info)
      )
