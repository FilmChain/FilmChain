...
namespace mcc { namespace blockchain {
   ...
   { try {
       ...
       if( pending_state()->get_head_block_num() >= MCC_V0_9_2_FORK_BLOCK_NUM )
       {
           for( const auto& op : trx.operations )
           {
               if( operation_type_enum( op.type ) == cover_op_type )
               {
                   FC_ASSERT( fees_paid[ 0 ] <= MCC_BLOCKCHAIN_PRECISION );
               }
           }
       }
   } FC_CAPTURE_AND_RETHROW() }
   ...
   { try {
      trx = trx_arg;
      try {
          if (pending_state()->get_head_block_num() != 0)
          {

              if (pending_state()->now() >= trx_arg.expiration)
              {

                  if (pending_state()->now() > trx_arg.expiration || pending_state()->get_head_block_num() >= MCC_V0_4_21_FORK_BLOCK_NUM)
                  {
        ...
        if( (pending_state()->now() + MCC_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration )
           FC_CAPTURE_AND_THROW( invalid_transaction_expiration, (trx_arg)(pending_state()->now()) );

        if( pending_state()->get_head_block_num() >= MCC_V0_4_26_FORK_BLOCK_NUM )
        {
   ...
   { try {
       ...
       if( slate_record->duplicate_slate.empty() )
       {
           for( const account_id_type id : slate_record->slate )
           {
			   if (id >= 0)
			   {
				   auto delegate_vote = fc::safe<share_type>(delegate_vote_deltas[id]);
				   delegate_vote += amount;
				   delegate_vote_deltas[id] = delegate_vote.value;
			   }
           }
       }
       else
   ...
   { try {
	   fc::safe<share_type> balance_t = fees_paid[amount.asset_id];
	   fc::safe<share_type> deltas_t = op_deltas[_current_op_index][amount.asset_id];

	   balance_t -= amount.amount;
	   deltas_t += amount.amount;
	   fees_paid[amount.asset_id] = balance_t.value;
	   op_deltas[_current_op_index][amount.asset_id] = deltas_t.value;
