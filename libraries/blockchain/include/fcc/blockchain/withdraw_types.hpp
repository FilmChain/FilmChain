...
namespace mcc { namespace blockchain {
   ...
   struct memo_data
   {
       ...
       */
      fc::array<char,MCC_BLOCKCHAIN_MAX_MEMO_SIZE>     message;
   ...
   struct extended_memo_data
   {
      ...
      fc::array<char,MCC_BLOCKCHAIN_MAX_MEMO_SIZE>      message;
      fc::enum_type<uint8_t,memo_flags_enum>            memo_flags = from_memo;
      fc::array<char,MCC_BLOCKCHAIN_EXTENDED_MEMO_SIZE> extra_message;
   };
...
namespace fc {
   void to_variant( const mcc::blockchain::withdraw_condition& var,  variant& vo );
   void from_variant( const variant& var,  mcc::blockchain::withdraw_condition& vo );
   void to_variant( const mcc::blockchain::memo_data& var,  variant& vo );
   void from_variant( const variant& var,  mcc::blockchain::memo_data& vo );
}

FC_REFLECT_ENUM( mcc::blockchain::withdraw_condition_types,
        (withdraw_null_type)
...
FC_REFLECT( mcc::blockchain::withdraw_condition,
        (asset_id)
        (slate_id)
        (type)
        (data)
        )
FC_REFLECT( mcc::blockchain::titan_memo,
        (one_time_key)
        (encrypted_memo_data)
        )
FC_REFLECT_ENUM( mcc::blockchain::memo_flags_enum,
        (from_memo)
        (to_memo)
        )
FC_REFLECT( mcc::blockchain::memo_data,
        (from)
        (from_signature)
        (message)
        (memo_flags)
...
FC_REFLECT( mcc::blockchain::extended_memo_data,
        (from)
        (from_signature)
        (message)
        (memo_flags)
...
FC_REFLECT_DERIVED( mcc::blockchain::memo_status,
        (mcc::blockchain::memo_data),
        (has_valid_signature)
        (owner_private_key)
        )
FC_REFLECT( mcc::blockchain::withdraw_with_signature,
        (owner)
        (memo)
        )
FC_REFLECT( mcc::blockchain::withdraw_vesting,
        (owner)
        (start_time)
        (duration)
        (original_balance)
        )
FC_REFLECT( mcc::blockchain::withdraw_with_multisig,
        (required)
        (owners)
        (memo)
        )
FC_REFLECT( mcc::blockchain::withdraw_with_escrow,
        (sender)
        (receiver)
        (escrow)
        (agreement_digest)
