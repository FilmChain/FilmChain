...
namespace mcc { namespace blockchain {
  ...
  struct price
  {
      ...
      fc::uint128_t ratio; // This is a DECIMAL FRACTION with denominator equal to MCC_PRICE_PRECISION
   ...
   *  denominator.  Therefore  fcc/usd and  usd/mcc will
   *  always result in a price measured in usd/mcc because
   ...
   *
   *  ie:  p = 3 usd/mcc & a = 4 mcc then result = 12 usd
   *  ie:  p = 3 usd/mcc & a = 4 usd then result = 1.333 mcc
   */
...
namespace fc
{
 //  void to_variant( const mcc::blockchain::asset& var,  variant& vo );
 //  void from_variant( const variant& var,  mcc::blockchain::asset& vo );
   void to_variant( const mcc::blockchain::price& var,  variant& vo );
   void from_variant( const variant& var,  mcc::blockchain::price& vo );
}

#include <fc/reflect/reflect.hpp>
FC_REFLECT( mcc::blockchain::asset, (amount)(asset_id) );
FC_REFLECT( mcc::blockchain::price, (ratio)(quote_asset_id)(base_asset_id) );
