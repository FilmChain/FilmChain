...
namespace mcc { namespace blockchain {
   ...
   class pending_chain_state : public chain_interface, public std::enable_shared_from_this<pending_chain_state>
   {
      public:
         ...
         void apply_asset_map(const chain_interface_ptr& prev_state) const ;
         ...
         unordered_map<balance_id_type, asset_id_type>                         _asset_balance_map_add;
         unordered_map<balance_id_type, asset_id_type>                         _asset_balance_map_remove;

         unordered_map<transaction_id_type, deposit_asset_record >          deposit_asset_trx_map_add;
         unordered_set<transaction_id_type>                                 deposit_asset_trx_map_remove;
      private:
         // Not serialized
         ...
         virtual odeposit_asset_record deposit_asset_record_lookup_by_trx_id(const transaction_id_type& trx_id)const override;
         virtual void deposit_asset_record_insert_into_trxid_map(const transaction_id_type& trx_id, const deposit_asset_record&) override;
         virtual void deposit_asset_record_erase_from_trxid_map(const transaction_id_type& trx_id) override;

         public:
         virtual void balance_insert_into_asset_map(const asset_id_type& asset_id, const balance_id_type& balance_id)override;
         virtual void balance_erase_from_asset_map(const asset_id_type& asset_id, const balance_id_type& balance_id)override;
         virtual set<balance_id_type> balance_map_lookup_by_asset_id(const asset_id_type& id) override;
   };
   typedef std::shared_ptr<pending_chain_state> pending_chain_state_ptr;

} } // mcc::blockchain

FC_REFLECT( mcc::blockchain::pending_chain_state,
            ...
            (market_transactions)
            (market_history)
            (_asset_balance_map_add)
            (_asset_balance_map_remove)
            (deposit_asset_trx_map_add)
            (deposit_asset_trx_map_remove)
            )
