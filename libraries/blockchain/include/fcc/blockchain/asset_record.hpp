...
struct deposit_asset_record;
typedef fc::optional<deposit_asset_record> odeposit_asset_record;
struct deposit_asset_record
{
    transaction_id_type                 trx_id;
    asset_id_type                       deposit_asset_id;
    map < balance_id_type, share_type > amounts;
    static odeposit_asset_record lookup(const chain_interface&, const transaction_id_type&);
    static void store(chain_interface&, const transaction_id_type&, const deposit_asset_record& );
    static void remove(chain_interface&, const transaction_id_type& );
};
class asset_db_interface
{
    friend struct asset_record;
    friend struct deposit_asset_record;
    ...
    virtual odeposit_asset_record deposit_asset_record_lookup_by_trx_id(const transaction_id_type& trx_id)const  = 0;
    virtual void deposit_asset_record_insert_into_trxid_map(const transaction_id_type& trx_id, const deposit_asset_record&) = 0;
    virtual void deposit_asset_record_erase_from_trxid_map(const transaction_id_type& trx_id) = 0;
...
FC_REFLECT_ENUM( mcc::blockchain::asset_record::flag_enum,
        (dynamic_max_supply)
        (dynamic_fees)
        (halted_markets)
        (halted_withdrawals)
...
FC_REFLECT( mcc::blockchain::asset_record,
        (id)
        (symbol)
        (issuer_id)
        (authority)
    ...
    FC_REFLECT(mcc::blockchain::deposit_asset_record,
        (trx_id)
        (deposit_asset_id)
        (amounts)
    )
