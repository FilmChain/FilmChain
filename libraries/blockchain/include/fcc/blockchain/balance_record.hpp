...
namespace mcc { namespace blockchain {
   ...
   public:
      virtual void balance_insert_into_asset_map(const asset_id_type& asset_id,const balance_id_type& balance_id)=0;
      virtual void balance_erase_from_asset_map(const asset_id_type& asset_id, const balance_id_type& balance_id)=0;

      virtual set<balance_id_type> balance_map_lookup_by_asset_id(const asset_id_type& id) =0;
   };

} } // mcc::blockchain

FC_REFLECT( mcc::blockchain::snapshot_record, (original_address)(original_balance) )
FC_REFLECT( mcc::blockchain::balance_record, (condition)(balance)(restricted_owner)(snapshot_info)(deposit_date)(last_update)(meta_data) )
