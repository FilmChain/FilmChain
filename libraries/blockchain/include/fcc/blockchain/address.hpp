...
namespace mcc { namespace blockchain {
   ...
   class address
   {
      public:
       ...
       static bool is_valid( const std::string& base58str, const std::string& prefix = MCC_ADDRESS_PREFIX );
...
namespace fc
{
   void to_variant( const mcc::blockchain::address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  mcc::blockchain::address& vo );
}

namespace std
{
   template<>
   struct hash<mcc::blockchain::address>
   {
       public:
         size_t operator()(const mcc::blockchain::address &a) const
         {
...
FC_REFLECT( mcc::blockchain::address, (addr) )
