...
struct account_record
{
    ...
    bool            is_titan_account()const {/* return meta_data.valid() && meta_data->type == titan_account;*/return false; }
...
FC_REFLECT_ENUM( mcc::blockchain::account_key_type,
                 (owner_key)
                 (active_key)
                 (signing_key)
                 )
FC_REFLECT_ENUM( mcc::blockchain::account_type,
                 (titan_account)
                 (public_account)
                 (multisig_account)
                 )
FC_REFLECT( mcc::blockchain::multisig_meta_info,
            (required)
            (owners)
            )
FC_REFLECT( mcc::blockchain::account_meta_info,
            (type)
            (data)
            )
FC_REFLECT( mcc::blockchain::delegate_stats,
            (votes_for)
            (pay_rate)
            (signing_key_history)
...
FC_REFLECT( mcc::blockchain::vote_del,
            (votes)
            (delegate_id)
            )
FC_REFLECT( mcc::blockchain::account_record,
            (id)
            (name)
            (public_data)
            (owner_key)
