...
namespace mcc { namespace blockchain {
   ...
   { try {
      ...
      FC_ASSERT(new_record.meta_data.valid() && new_record.meta_data->type != titan_account);
   ...
   { try {
      if( eval_state.pending_state()->get_head_block_num() < MCC_V0_6_0_FORK_BLOCK_NUM )
   ...
   { try {
      ...
      if( !eval_state.account_or_any_parent_has_signed( *account ) )
          FC_CAPTURE_AND_THROW( missing_signature, (*this) );
	  fc::safe<share_type> pay_balance = account->delegate_info->pay_balance;
      
	  pay_balance -= this->amount;
      account->delegate_info->pay_balance = pay_balance.value;
	  fc::safe<share_type> delegate_vote_deltas = eval_state.delegate_vote_deltas[account_id];
