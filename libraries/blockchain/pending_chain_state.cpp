...
namespace mcc { namespace blockchain {
   ...
   void pending_chain_state::apply_changes()const
   {
      ...
      apply_records( prev_state, deposit_asset_trx_map_add, deposit_asset_trx_map_remove);
      apply_asset_map(prev_state);
      ...
      for( const auto& item : market_history )  
          prev_state->store_market_history_record( item.first, item.second );
   ...
   void pending_chain_state::build_undo_state( const chain_interface_ptr& undo_state_arg )const
   {
      ...
      populate_undo_state( undo_state, prev_state, deposit_asset_trx_map_add, deposit_asset_trx_map_remove);
      
      undo_state->_asset_balance_map_add = _asset_balance_map_remove;
      undo_state->_asset_balance_map_remove = _asset_balance_map_add;
   ...
   void pending_chain_state::apply_asset_map(const chain_interface_ptr & prev_state) const 
   {
       for (auto to_remove = _asset_balance_map_remove.begin(); to_remove != _asset_balance_map_remove.end(); to_remove++)
       {
           prev_state->balance_erase_from_asset_map(to_remove->second, to_remove->first);
       }
       for (auto to_add = _asset_balance_map_add.begin(); to_add != _asset_balance_map_add.end(); to_add++)
       {
           prev_state->balance_insert_into_asset_map(to_add->second, to_add->first);
       }   
   }
   ...
   odeposit_asset_record pending_chain_state::deposit_asset_record_lookup_by_trx_id(const transaction_id_type & trx_id) const
   {
       const auto iter = deposit_asset_trx_map_add.find(trx_id);
       if (iter != deposit_asset_trx_map_add.end()) return iter->second;
       if (deposit_asset_trx_map_remove.count(trx_id) > 0) return odeposit_asset_record();
       const chain_interface_ptr prev_state = _prev_state.lock();
       if (!prev_state) return odeposit_asset_record();
       return prev_state->lookup<deposit_asset_record>(trx_id);
   }

   void pending_chain_state::deposit_asset_record_insert_into_trxid_map(const transaction_id_type & trx_id, const deposit_asset_record &rec)
   {
       deposit_asset_trx_map_remove.erase(trx_id);
       deposit_asset_trx_map_add.insert(std::make_pair(trx_id,rec));
   }

   void pending_chain_state::deposit_asset_record_erase_from_trxid_map(const transaction_id_type & trx_id)
   {
       deposit_asset_trx_map_add.erase(trx_id);
       deposit_asset_trx_map_remove.insert(trx_id);
   }

   void pending_chain_state::balance_insert_into_asset_map(const asset_id_type & asset_id, const balance_id_type & balance_id)
   {
       _asset_balance_map_remove.erase(balance_id);
       _asset_balance_map_add[balance_id] = asset_id;
   }

   void pending_chain_state::balance_erase_from_asset_map(const asset_id_type & asset_id, const balance_id_type & balance_id)
   {
       _asset_balance_map_add.erase(balance_id);
       _asset_balance_map_remove[balance_id] = asset_id;
   }

   set<balance_id_type> pending_chain_state::balance_map_lookup_by_asset_id(const asset_id_type & id)
   {
       set<balance_id_type> res;
       const chain_interface_ptr prev_state = _prev_state.lock();
       if (prev_state)
       {
           res = prev_state->balance_map_lookup_by_asset_id(id);
           for (auto to_remove = _asset_balance_map_remove.begin(); to_remove != _asset_balance_map_remove.end(); to_remove++)
           {
               if (to_remove->second == id)
                   res.erase(to_remove->first);
           }
       }
       for (auto to_add = _asset_balance_map_add.begin(); to_add != _asset_balance_map_add.end(); to_add++)
       {
           if (to_add->second == id)
               res.insert(to_add->first);
       }
       return res;
   }
