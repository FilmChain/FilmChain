...
namespace mcc { namespace blockchain {
   ...
   { try {
      if( name.size() < MCC_BLOCKCHAIN_MIN_NAME_SIZE ) return false;
      if( name.size() > MCC_BLOCKCHAIN_MAX_NAME_SIZE ) return false;
   ...
   { try {
       if( get_head_block_num() < MCC_V0_5_0_FORK_BLOCK_NUM )
           return is_valid_symbol_name_v1( symbol );

       FC_ASSERT( symbol != "FCCX" );

       if( symbol.size() < MCC_BLOCKCHAIN_MIN_SYMBOL_SIZE)
       ...
       if (dots == 0)
       {
         if (position > MCC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
           FC_ASSERT(false, "Symbol name too large");
       }
       else //dots == 1 means hierarchial asset name
       {
         if (position - dot_position - 1> MCC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
           FC_ASSERT(false, "Sub-symbol name too large");
         if( symbol.size() > MCC_BLOCKCHAIN_MAX_SYMBOL_SIZE)
           FC_ASSERT(false, "Symbol name too large");
       }
   ...
   {
	   return share_type(0);
       share_type pay_per_block = MCC_MAX_DELEGATE_PAY_PER_BLOCK;
   ...
   share_type chain_interface::get_delegate_registration_fee( uint8_t pay_rate )const
   {
       ...
       const share_type max_pay_per_two_weeks = max_total_pay_per_two_weeks / MCC_BLOCKCHAIN_NUM_DELEGATES;
   ...
   {
       return 10 * MCC_BLOCKCHAIN_PRECISION;
       //if( get_head_block_num() < MCC_V0_9_0_FORK_BLOCK_NUM )
       //    return get_asset_registration_fee_v2( symbol_length );
       //
       //// TODO: Add #define's for these fixed prices
       //static const share_type long_symbol_price = 5000 * MCC_BLOCKCHAIN_PRECISION; // $100 at $0.02/share
   ...
   { try {
       ...
       {
           if (record.balance > 0)
               balance_insert_into_asset_map(record.asset_id(), record.id());
           else
               balance_erase_from_asset_map(record.asset_id(), record.id());
       }
   } FC_CAPTURE_AND_RETHROW( (record) ) }
   ...
   void chain_interface::store_deposit_to_asset_record(const deposit_asset_record & record)
   {
       store(record.trx_id,record);
   }

   odeposit_asset_record chain_interface::get_deposit_to_asset_record(const transaction_id_type & id) const
   {
       return lookup<deposit_asset_record, transaction_id_type>(id);
   }
