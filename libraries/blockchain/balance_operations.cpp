...
namespace mcc { namespace blockchain {
   ...
   asset balance_record::calculate_yield( fc::time_point_sec now, share_type amount, share_type yield_pool, share_type share_supply )const
   {
      ...
      if( elapsed_time <= fc::seconds( MCC_BLOCKCHAIN_MIN_YIELD_PERIOD_SEC ) )
      ...
      // MCC_BLOCKCHAIN_MAX_SHARES * 1000000 * MCC_BLOCKCHAIN_MAX_SHARES
       ...
       *  interest.
       */
      if( elapsed_time < fc::seconds( MCC_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC ) )
      {
         ...
         // MCC_BLOCKCHAIN_MAX_SHARES * 1000000 * MCC_BLOCKCHAIN_MAX_SHARES * 8
         // thus it cannot overflow
         //
         ...
         yield /= MCC_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC;

         delta_yield *= elapsed_time.to_seconds();
         delta_yield /= MCC_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC;
   ...
   { try {
	   ...
	   fc::safe<share_type> temp = cur_record->balance;
	   temp += this->amount;
	   cur_record->balance = temp.value;
   ...
   { try {
      ...
      if( eval_state.pending_state()->get_head_block_num() < MCC_V0_4_21_FORK_BLOCK_NUM )
      ...
      FC_ASSERT( asset_rec.valid() );
      FC_ASSERT(current_balance_record->condition.asset_id == asset_id);
   ...
   { try {
      ...
      if( amount.asset_id == 0 )
      {
          ...
          const share_type required_fee = message_kb * MCC_BLOCKCHAIN_MIN_BURN_FEE;
   ...
   { try {
	  ...
	  auto temp = fc::safe<share_type>(amount_to_sender);
	  temp += fc::safe<share_type>(amount_to_receiver);
	  uint64_t total_released = uint64_t(temp.value);
   ...
   { try {
      ...
      FC_ASSERT( current_balance_record->condition.asset_id == 0, "Only MCC balances can have restricted owners." );
      ...
      auto fee = MCC_BLOCKCHAIN_PRECISION / 2;
      ...
      else // NOT this->new_restricted_owner.valid() || (this->new_restricted_owner == new_restricted_owner)
      {
          ...
          FC_ASSERT( eval_state.pending_state()->now().sec_since_epoch() - last_update_secs
                     >= MCC_BLOCKCHAIN_VOTE_UPDATE_PERIOD_SEC,
