...
namespace mcc { namespace blockchain {
   ...
   void transaction::withdraw( const balance_id_type& account,
                               share_type             amount ,const asset_id_type& asset_id)
   { try {
      FC_ASSERT( amount > 0, "amount: ${amount}", ("amount",amount) );
      operations.emplace_back( withdraw_operation( account, amount , asset_id) );
   ...
   void transaction::deposit_to_asset(const asset & deposit_asset, const asset_id_type & target_asset)
   {
       deposit_to_asset_operation op;
       op.deposit_amount = deposit_asset.amount;
       op.target_asset = target_asset;
       op.deposit_asset = deposit_asset.asset_id;
   ...
   bool transaction::is_cancel()const
   {
    ...
    void transaction::uia_update_authority(const asset_id_type asset_id, const multisig_meta_info& multisig)
    {
        try
        {
            asset_update_permissions_operation op;
            op.asset_id = asset_id;
            op.authority = multisig;

            operations.emplace_back(std::move(op));

        }FC_CAPTURE_AND_RETHROW((asset_id)(multisig))
