...
{ try {
    ...
    {
		if ((int32_t)quote_asset_id <= base_asset_id)
            FC_CAPTURE_AND_THROW( invalid_price );
        ...
        if( eval_state.pending_state()->get_head_block_num() >= MCC_V0_9_1_FORK_BLOCK_NUM )
	...
	auto temp = fc::safe<share_type>(current_order->balance);
	temp+= delta_amount.amount;
	current_order->balance = temp.value;
...
{ try {
    ...
    const time_point_sec start_time = current_position->expiration - fc::seconds( MCC_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC );
    ...
    else
    {
        ...
        const price new_call_price = asset( current_position->payoff_balance, this->cover_index.order_price.quote_asset_id )
                                     / asset( (current_position->collateral_balance * MCC_BLOCKCHAIN_MCALL_D2C_NUMERATOR)
                                              / MCC_BLOCKCHAIN_MCALL_D2C_DENOMINATOR, this->cover_index.order_price.base_asset_id );
	...
	fc::safe<share_type> temp = fc::safe<share_type>(quote_asset_record->current_supply);
	fc::safe<share_type> temp2 = fc::safe<share_type>(quote_asset_record->collected_fees);
	temp -= principle_paid.amount;
	quote_asset_record->current_supply = temp.value;
	temp2 += interest_paid.amount;
	quote_asset_record->collected_fees = temp2.value;
...
{ try {
    ...
    const address& owner = this->cover_index.owner;
	fc::safe<share_type> temp = current_position->collateral_balance;
	temp += delta_amount.amount;
	current_position->collateral_balance = temp.value;
