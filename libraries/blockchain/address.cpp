...
namespace mcc {
  namespace blockchain {
   ...
   address::address( const std::string& base58str )
   {
      ...
      if( is_valid( base58str, "FCCX" ) ) prefix = std::string( "FCCX" );
   ...
   {
      if( prefix == MCC_ADDRESS_PREFIX && is_valid( base58str, "FCCX" ) )
          return true;
   ...
   address::address( const mcc::blockchain::public_key_type& pub )
   {
   ...
   address::operator std::string()const
   {
        ...
        return MCC_ADDRESS_PREFIX + fc::to_base58( bin_addr.data, sizeof( bin_addr ) );
   }

} } // namespace mcc::blockchain

namespace fc
{
    void to_variant( const mcc::blockchain::address& var,  variant& vo )
    {
        vo = std::string(var);
    }
    void from_variant( const variant& var,  mcc::blockchain::address& vo )
    {
        vo = mcc::blockchain::address( var.as_string() );
    }
}
