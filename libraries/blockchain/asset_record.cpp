...
namespace mcc { namespace blockchain {
    ...
    { try {
        ...
        if( satoshis > MCC_BLOCKCHAIN_MAX_SHARES )
            FC_CAPTURE_AND_THROW( amount_too_large, (satoshis)(MCC_BLOCKCHAIN_MAX_SHARES) );
    ...
    { try {
        ...
        FC_ASSERT( id == 0 || issuer_id == market_issuer_id || db.lookup<account_record>( issuer_id ).valid()|| issuer_id==asset_id_type());
        ...
        FC_ASSERT( max_supply >= 0 && max_supply <= MCC_BLOCKCHAIN_MAX_SHARES );
        FC_ASSERT( withdrawal_fee >= 0 && withdrawal_fee <= max_supply );
        FC_ASSERT( market_fee_rate <= MCC_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE );
    ...
    odeposit_asset_record blockchain::deposit_asset_record::lookup(const chain_interface & db, const transaction_id_type&id)
    {
        try {
        return db.deposit_asset_record_lookup_by_trx_id(id);
        } FC_CAPTURE_AND_RETHROW((id))
    }
    void deposit_asset_record::store(chain_interface & db, const transaction_id_type& tid, const deposit_asset_record& rec)
    {
        try {
        db.deposit_asset_record_insert_into_trxid_map(tid, rec);
        } FC_CAPTURE_AND_RETHROW((rec))
    }
    void deposit_asset_record::remove(chain_interface &db, const transaction_id_type& id)
    {
        try {
        db.deposit_asset_record_erase_from_trxid_map(id);
        } FC_CAPTURE_AND_RETHROW((id))
    }
}
