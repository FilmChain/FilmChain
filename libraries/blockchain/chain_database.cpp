...
#include <fcc/blockchain/balance_operations.hpp>
#include <fcc/blockchain/chain_database.hpp>
#include <fcc/blockchain/chain_database_impl.hpp>
#include <fcc/blockchain/checkpoints.hpp>
#include <fcc/blockchain/config.hpp>
#include <fcc/blockchain/exceptions.hpp>
#include <fcc/blockchain/genesis_state.hpp>
#include <fcc/blockchain/genesis_json.hpp>
#include <fcc/blockchain/market_engine.hpp>
#include <fcc/blockchain/snapshot_state.hpp>
#include <fcc/blockchain/time.hpp>
...
namespace mcc { namespace blockchain {
   ...
   namespace detail
   {
      ...
      { try {
          ...
          if( !version_record.valid() || version_record->value.as_uint64() != MCC_BLOCKCHAIN_DATABASE_VERSION )
              return true;
      ...
      { try {
          ...
          _asset_balance_map.open(data_dir / "index/asset_balance_map");
          deposit_asset_trx_map.open(data_dir / "index/deposit_asset_trx_map");
          _pending_trx_state = std::make_shared<pending_chain_state>( self->shared_from_this() );
      ...
      { try {
         ...
         if( chain_id == MCC_EXPECTED_CHAIN_ID )
             chain_id = MCC_DESIRED_CHAIN_ID;

         self->set_chain_id( chain_id );

         // Check genesis state
         FC_ASSERT( config.delegates.size() >= MCC_BLOCKCHAIN_NUM_DELEGATES,
                    "genesis.json does not contain enough initial delegates!",
                    ("required",MCC_BLOCKCHAIN_NUM_DELEGATES)("provided",config.delegates.size()) );
         ...
         {
             static const vector<string> mcc_prefixes{ "FCC", "KEY", "DVS", "XTS" };
             try
             {
             ...
             {
                 for( const string& prefix : mcc_prefixes )
                 {
                     if( raw_address.find( prefix ) == 0 )
                     {
                         return address( MCC_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                     }
                 }
             }
         ...
         base_asset.symbol = MCC_BLOCKCHAIN_SYMBOL;
         base_asset.issuer_id = asset_record::god_issuer_id;
         base_asset.name = MCC_BLOCKCHAIN_NAME;
         base_asset.description = MCC_BLOCKCHAIN_DESCRIPTION;
         base_asset.precision = MCC_BLOCKCHAIN_PRECISION;
         base_asset.max_supply = MCC_BLOCKCHAIN_MAX_SHARES;
         ...
         for( const genesis_asset& asset : config.market_assets )
         {
             ...
             rec.max_supply = MCC_BLOCKCHAIN_MAX_SHARES;
      ...
      { try {
          if( pending_state->get_head_block_num() < MCC_V0_6_0_FORK_BLOCK_NUM )
          ...
          static const uint32_t blocks_per_two_weeks = 14 * MCC_BLOCKCHAIN_BLOCKS_PER_DAY;
      ...
      { try {
          ...
          if( block_num > MCC_BLOCKCHAIN_MAX_UNDO_HISTORY )
          {
              const uint32_t old_block_num = block_num - MCC_BLOCKCHAIN_MAX_UNDO_HISTORY;
      ...
      { try {
          ...
          if( block_digest.timestamp.sec_since_epoch() % MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0 )
          ...
          fc::time_point_sec now = mcc::blockchain::now();
          const uint32_t delta_seconds = (block_digest.timestamp - now).to_seconds();
          if( block_digest.timestamp >  (now + MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC*2) )
             FC_CAPTURE_AND_THROW( time_in_future, (block_digest.timestamp)(now)(delta_seconds) );

          if( block_digest.block_num >= MCC_V0_9_0_FORK_BLOCK_NUM )
          {
      ...
      /**
       *  A block should be produced every MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
      ...
      { try {
          ...
          if( head_block.block_num > 0 ) block_timestamp = head_block.timestamp + MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
          else block_timestamp = block_header.timestamp;
          const auto& active_delegates = self->get_active_delegates();

          for( ; block_timestamp < block_header.timestamp; block_timestamp += MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
          {
      ...
      void chain_database_impl::update_active_delegate_list( const uint32_t block_num,
                                                             const pending_chain_state_ptr& pending_state )const
      { try {
          if( pending_state->get_head_block_num() < MCC_V0_9_0_FORK_BLOCK_NUM )
              return update_active_delegate_list_v1( block_num, pending_state );

          if( block_num % MCC_BLOCKCHAIN_NUM_DELEGATES != 0 )
              return;
          ...
          const pending_chain_state_ptr& pending_state ) const
      {
          ...
          for( mcc::db::cached_level_map< market_index_key, order_record >::iterator
          ...
          for( mcc::db::cached_level_map< market_index_key, collateral_record >::iterator
          ...
          for( auto it=mkt_to_best_bid.begin(); it != mkt_to_best_bid.end(); it++ )
          {
              ...
              auto get_order_type = [&]( const market_order& order ) -> std::string
              {
                  switch( order.type )
                  {
                      case mcc::blockchain::null_order:
                          return "null_order";
                      case mcc::blockchain::bid_order:
                          return "bid_order";
                      case mcc::blockchain::ask_order:
                          return "ask_order";
                      case mcc::blockchain::short_order:
                          return "short_order";
                      case mcc::blockchain::cover_order:
                          return "cover_order";
                      default:
                          return "<unknown>";
                  }
      ...
      { try {
         ...
         try
         {
            ...
            if( block_data.block_num < MCC_V0_4_9_FORK_BLOCK_NUM )
            ...
            if( block_data.block_num == MCC_V0_4_16_FORK_BLOCK_NUM )
            {
            ...
            else if( block_data.block_num == MCC_V0_4_17_FORK_BLOCK_NUM
                     || block_data.block_num == MCC_V0_4_21_FORK_BLOCK_NUM
                     || block_data.block_num == MCC_V0_4_24_FORK_BLOCK_NUM
                     || block_data.block_num == MCC_V0_9_0_FORK_BLOCK_NUM )
            {
                ...
                auto demcc = self->calculate_demcc( false );
                ...
                for( auto& record : records )
                {
                    ...
                    {
                        share_type debt = demcc[ record.id ];
                        if( supply != debt )
                        {
            ...
            if( block_data.block_num == MCC_V0_4_24_FORK_BLOCK_NUM )
            {
            ...
            if( block_data.block_num == MCC_V0_9_0_FORK_BLOCK_NUM )
            {
                static const fc::uint128 max_apr = fc::uint128( MCC_BLOCKCHAIN_MAX_SHORT_APR_PCT ) * FC_REAL128_PRECISION / 100;
         ...
         if( (blockchain::now() - block_data.timestamp).to_seconds() < MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
         {
      ...
      { try {
         ...
         mcc::blockchain::pending_chain_state_ptr undo_state_ptr = std::make_shared<mcc::blockchain::pending_chain_state>( undo_state );
   ...
   {
       return get_delegates_by_vote( 0, MCC_BLOCKCHAIN_NUM_DELEGATES );
   }
   ...
   { try {
      ...
      try
      {
          ...
          else
          {
              ...
              my->open_database( data_dir );
              store_property_record( property_id_type::database_version, variant( MCC_BLOCKCHAIN_DATABASE_VERSION ) );
              ...
              const auto toggle_leveldb = [ this ]( const bool enabled )
              {
                  ...
                  my->_asset_balance_map.toggle_leveldb(enabled);
                  my->deposit_asset_trx_map.toggle_leveldb(enabled);
              };
              ...
              const auto insert_block = [&]( const full_block& block )
              {
                  if( blocks_indexed % 200 == 0 )
                  {
                      ...
                      else
                      {
                          ...
                          progress = float( blocks_indexed * MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC ) / seconds;
                      }
              ...
              else
              {
                  ...
                  if( last_known_block_num > MCC_BLOCKCHAIN_MAX_UNDO_HISTORY )
                      my->_min_undo_block = last_known_block_num - MCC_BLOCKCHAIN_MAX_UNDO_HISTORY;
   ...
   { try {
      ...
      my->_asset_balance_map.close();
      my->deposit_asset_trx_map.close();
   } FC_CAPTURE_AND_RETHROW() }
   ...
   { try {
      ...
      auto delegate_pos = slot_number % MCC_BLOCKCHAIN_NUM_DELEGATES;
   ...
   { try {
      ...
      if( next_block_time <= now() ) next_block_time += MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
      auto last_block_time = next_block_time + (MCC_BLOCKCHAIN_NUM_DELEGATES * MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

      auto active_delegates = get_active_delegates();
      for( ; next_block_time < last_block_time; next_block_time += MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
      {
   ...
   { try {
      ...
      if( head_block_num > MCC_BLOCKCHAIN_MAX_UNDO_HISTORY &&
          block_data.block_num <= (head_block_num - MCC_BLOCKCHAIN_MAX_UNDO_HISTORY) )
      {
        elog( "block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
               "our undo history would allow us to switch to (current head block is number ${head_block_num}, undo history is ${undo_history})",
                           ...
                           ("head_block_num", head_block_num)("undo_history", MCC_BLOCKCHAIN_MAX_UNDO_HISTORY));
  ...
  std::vector<block_id_type> chain_database::get_fork_history( const block_id_type& id )
  {
   ...
   { try {
      ...
      if( !override_limits )
      {
         if( my->_pending_fee_index.size() > MCC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE )
         {
             auto overage = my->_pending_fee_index.size() - MCC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
   ...
   { try {
      const time_point start_time = time_point::now();
      //FC_ASSERT( block_timestamp < (fc::time_point_sec() + fc::seconds( 1444741200 )), "Upgrade to MCC 2.0" );

      const pending_chain_state_ptr pending_state = std::make_shared<pending_chain_state>( shared_from_this() );
      if( pending_state->get_head_block_num() >= MCC_V0_4_9_FORK_BLOCK_NUM )
   ...
   { try {
    ...
    string chain_database::export_fork_graph( uint32_t start_block, uint32_t end_block, const fc::path& filename )const
    {
      ...
      for( auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr )
      {
        ...
        {
          unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
   ...
   { try {
      ...
      const auto now = mcc::blockchain::now();
      if( head_num < 1 )
      {
          return 0;
      }
      else if( head_num <= MCC_BLOCKCHAIN_NUM_DELEGATES )
      {
         // what percent of the maximum total blocks that could have been produced
         // have been produced.
         const auto expected_blocks = (now - get_block_header( 1 ).timestamp).to_seconds() / MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
      ...
      else
      {
         ...
         const auto starting_time = get_block_header( head_num - MCC_BLOCKCHAIN_NUM_DELEGATES ).timestamp;
         const auto expected_production = (now - starting_time).to_seconds() / MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
         return 100*double( MCC_BLOCKCHAIN_NUM_DELEGATES ) / expected_production;
      }
   } FC_RETHROW_EXCEPTIONS( warn, "" ) }
                                                                     ...
                                                                     const address& owner)
   {
      ...
      uint32_t old_current_block_num = current_block_num;
      bool dec = false;
      ...
      fc::time_point_sec stamp = get_block_header(old_current_block_num).timestamp;
      ...
      while( --current_block_num >= 1 && orders.size() < limit )
      {
        ...
        stamp = get_block_header(old_current_block_num).timestamp;
   ...
   { try {
       ...
       snapshot.max_core_supply = calculate_max_core_supply( MCC_MAX_DELEGATE_PAY_PER_BLOCK );
       ...
       const auto scan_account = [ & ]( const account_record& record )
       {
           ...
           if( record.is_delegate() )
           {
               ...
               {
                   snapshot_account.daily_pay = MCC_BLOCKCHAIN_BLOCKS_PER_DAY * MCC_MAX_DELEGATE_PAY_PER_BLOCK * record.delegate_info->pay_rate
                                                / MCC_BLOCKCHAIN_NUM_DELEGATES / 100;
               }
           }
       };
       ...
       for( auto iter = my->_collateral_db.begin(); iter.valid(); ++iter )
       {
           ...
           snapshot_asset.demcc[ index.owner ].collateral += collateral.collateral_balance;
           snapshot_asset.demcc[ index.owner ].debt += collateral.payoff_balance;
       }
       ...
       for( auto& item : snapshot.assets )
       {
           ...
           for( const auto& item : asset.demcc )
           {
           ...
           if( total_supply != total_debt )
           {
               ...
               for( auto& item : debt_list )
                   asset.demcc[ item.first ] = item.second;
       ...
       for( const auto& item : snapshot.assets )
       {
           ...
           snapshot.summary.num_collateral_positions += asset.demcc.size();
       }
       ...
       for( const auto& item : snapshot.balances )
       {
           ...
           if( balances.vesting.valid() )
           {
               ...
               vesting_balance.asset_symbol = MCC_BLOCKCHAIN_SYMBOL;
       ...
       for( const auto& item : snapshot.assets )
       {
           ...
           for( const auto& debt_item : snapshot_asset.demcc )
           {
   ...
   unordered_map<asset_id_type, share_type> chain_database::calculate_demcc( bool include_interest )const
   { try {
       unordered_map<asset_id_type, share_type> totals;
       ...
       for( auto iter = my->_collateral_db.begin(); iter.valid(); ++iter )
       {
           ...
           if( include_interest )
           {
               const time_point_sec position_start_time = collateral.expiration - MCC_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC;
   ...
   { try {
       ...
       for( ; pay_per_block > 0; pay_per_block /= 2 )
       {
           ...
           max_supply += interval_duration * pay_per_block / MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
   ...
   { try {
       ...
       if( prices.size() >= MCC_BLOCKCHAIN_MIN_FEEDS )
       {
   ...
   odeposit_asset_record chain_database::deposit_asset_record_lookup_by_trx_id(const transaction_id_type & trx_id) const
   {
       auto out=my->deposit_asset_trx_map.unordered_find(trx_id);
       if (out != my->deposit_asset_trx_map.unordered_end())
           return out->second;
       return odeposit_asset_record();
   }

   void chain_database::deposit_asset_record_insert_into_trxid_map(const transaction_id_type & trx_id, const deposit_asset_record &rec)
   {
       my->deposit_asset_trx_map.store(trx_id, rec);
   }

   void chain_database::deposit_asset_record_erase_from_trxid_map(const transaction_id_type & trx_id)
   {
       my->deposit_asset_trx_map.remove(trx_id);
   }

   void chain_database::balance_insert_into_asset_map(const asset_id_type & asset_id, const balance_id_type & balance_id)
   {
       auto out=my->_asset_balance_map.unordered_find(asset_id);
       if(out== my->_asset_balance_map.unordered_end())
       {
           set<balance_id_type> balance_map;
           balance_map.insert(balance_id);
           my->_asset_balance_map.store(asset_id, balance_map);
           return;
       }
       auto new_set=out->second;
       if(new_set.insert(balance_id).second == true)
           my->_asset_balance_map.store(asset_id, new_set);
   }

   void chain_database::balance_erase_from_asset_map(const asset_id_type & asset_id, const balance_id_type & balance_id)
   {
       auto out = my->_asset_balance_map.unordered_find(asset_id);
       if (out == my->_asset_balance_map.unordered_end())
       {
       ...
       auto new_set = out->second;
       if (new_set.erase(balance_id))
           my->_asset_balance_map.store(asset_id, out->second);
   }

   set<balance_id_type> chain_database::balance_map_lookup_by_asset_id(const asset_id_type & id) 
   {
       auto out = my->_asset_balance_map.unordered_find(id);
       if (out == my->_asset_balance_map.unordered_end())
        return set<balance_id_type>();
       return out->second;
   }
