...
#define MCC_LIGHT_WALLET_PORT 8899
#define MCC_LIGHT_WALLET_STORAGE_VERSION 1
...
namespace mcc { namespace light_wallet {
   ...
   class light_wallet
   {
      public:
         ...
         mcc::wallet::transaction_ledger_entry summarize(const std::string& account_name, const fc::variant_object& transaction_bundle);
...
FC_REFLECT( mcc::light_wallet::account_data,
            (encrypted_private_key)
            (user_account)
            (transaction_record_cache)
            (transaction_index)
            (scan_cache) );
FC_REFLECT( mcc::light_wallet::light_wallet_data,
