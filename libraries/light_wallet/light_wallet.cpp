#include <fcc/light_wallet/light_wallet.hpp>
#include <fcc/blockchain/time.hpp>
#include <fcc/blockchain/balance_operations.hpp>
#include <fcc/blockchain/transaction_creation_state.hpp>
...
namespace mcc { namespace light_wallet {
using std::string;
     ...
     restore(restore_function)
{
   try {
      ...
      if( storage_version != MCC_LIGHT_WALLET_STORAGE_VERSION )
      {
      ...
      persist("storage_version", fc::to_string(MCC_LIGHT_WALLET_STORAGE_VERSION));
...
{
   string last_error;
   auto resolved = fc::resolve( host, port ? port : MCC_LIGHT_WALLET_PORT );
...
{ try {
   ...
   mcc::blockchain::public_key_type recipient_key;
   if( to_account.valid() )
      recipient_key = to_account->active_key();
   else
      recipient_key = mcc::blockchain::public_key_type(to_account_name);

   asset fee = get_fee( symbol );

   fc::time_point_sec expiration = fc::time_point::now() + fc::seconds(MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 30);

   mcc::blockchain::transaction_creation_state creator;
...
oprice light_wallet::get_median_feed_price( const string& symbol )
{
   ...
   if( price_ratio != "" )
   {
      auto base_rec = get_asset_record( MCC_BLOCKCHAIN_SYMBOL );
...
map<string, pair<double,double>> light_wallet::balance(const string& account_name) const
{
   ...
   map<string, pair<double,double>> balances = {{MCC_BLOCKCHAIN_SYMBOL, {0,0}}};
...
vector<mcc::wallet::transaction_ledger_entry> light_wallet::transactions(const string& account_name,
                                                                         const string& symbol)
{
   FC_ASSERT( is_open() );
   ...
   vector<mcc::wallet::transaction_ledger_entry> results;
...
mcc::wallet::transaction_ledger_entry light_wallet::summarize(const string& account_name,
                                                              const fc::variant_object& transaction_bundle)
{ try {
   FC_ASSERT( is_open() );
   FC_ASSERT( is_unlocked() );
   ...
   mcc::wallet::transaction_ledger_entry summary;
