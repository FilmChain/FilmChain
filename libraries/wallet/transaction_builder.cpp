...
void transaction_builder::set_wallet_implementation(std::unique_ptr<mcc::wallet::detail::wallet_impl>& wimpl)
{
    _wimpl = wimpl.get();
}
...
transaction_builder& transaction_builder::deposit_asset_with_escrow(const mcc::wallet::wallet_account_record& payer,
                                                        const mcc::blockchain::account_record& recipient,
                                                        const mcc::blockchain::account_record& escrow_agent,
...
{ try {
   ...
   if( memo.size() > MCC_BLOCKCHAIN_MAX_MEMO_SIZE )
...
{ try {
   ...
   auto age_at_earliest_confirmation = _wimpl->_blockchain->now()
                                       + MCC_BLOCKCHAIN_BLOCK_INTERVAL_SEC
                                       - (*order->expiration - MCC_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC);
...
{ try {
   ...
   if (count == required_signatures.size())
       required_signatures_signed = true;
   for (const auto& address : optional_signatures)
   {
       if (required_signatures.find(address) != required_signatures.end())
           continue;
       //Ignore exceptions; this function operates on a best-effort basis, and doesn't actually have to succeed.
       try {
           ilog("@n trying to sign for address ${a}", ("a", address));
   ...
   const auto trx_digest = trx.digest(_wimpl->_blockchain->get_chain_id());
   std::set<address>signed_addresses;
   ...
   int optional_signed = 0;
   int required_signed = 0;
   for (auto addr : optional_signatures)
   {
       if (signed_addresses.find(addr) != signed_addresses.end())
       {
           ++optional_signed;
       }
       else
       {
           signatures_unsigned.insert(addr);
       }
   }
   for (auto addr : required_signatures)
   {
       if (signed_addresses.find(addr) != signed_addresses.end())
       {
           ++required_signed;
   ...
   optional_signatures_enough = false;
   if (optional_signed >= optional_signatures_required)
   {
       if (required_signed >= required_signatures.size())
           optional_signatures_enough = true;
   }
   return transaction_record;
} FC_CAPTURE_AND_RETHROW() }

bool mcc::wallet::transaction_builder::is_signed() const
{
    return optional_signatures_enough;
}

transaction_builder & mcc::wallet::transaction_builder::uia_issue_to_asset(const asset & issue_asset, const asset_id_type & target_asset_id)
{
    auto asset_rec=_wimpl->_blockchain->get_asset_record(issue_asset.asset_id);
    if (!asset_rec.valid())
        FC_CAPTURE_AND_THROW(unknown_asset_id,(issue_asset));
    optional_signatures_required = asset_rec->authority.required;
    for (const address& authority : asset_rec->authority.owners)
        optional_signatures.insert(authority);
    trx.deposit_to_asset(issue_asset, target_asset_id);
    trx.uia_issue(issue_asset);
    trx.expiration = blockchain::now() + _wimpl->self->get_transaction_expiration();
    return *this;
}

void mcc::wallet::transaction_builder::withdraw_transaction_fee(const string & account, const asset & amount)
{
    _wimpl->withdraw_to_transaction(amount, account,trx, required_signatures);
}

transaction_builder & mcc::wallet::transaction_builder::operator=(transaction_builder &builder)
{
    ...
    optional_signatures = builder.optional_signatures;
    optional_signatures_required = builder.optional_signatures_required;
    optional_signatures_enough = builder.optional_signatures_enough;
    _wimpl = builder._wimpl;
    signatures_unsigned = builder.signatures_unsigned;
    return *this;
}
...
{ try {
    ...
    if( obalance.valid() )
    {
        trx.withdraw( from, amount, obalance->asset_id());
    ...
    } else // We go ahead and try to use this balance ID as an owner
    {
        ...
        trx.withdraw( balance.id(), amount , balance.asset_id());
...
transaction_builder mcc::wallet::transaction_builder::uia_issue_or_collect_fees(const bool issue_new, const asset & amount, const string & generic_recipient, const string & memo, const bool sign)
{


    const wallet_contact_record recipient = _wimpl->generic_recipient_to_contact(generic_recipient);

    trx.expiration = blockchain::now() + _wimpl->self->get_transaction_expiration();

    const auto required_fees = _wimpl->self->get_transaction_fee();

    const oasset_record asset_record = _wimpl->_blockchain->get_asset_record(amount.asset_id);

    FC_ASSERT(asset_record.valid());
    owallet_account_record issuer_account;
    bool get_withdraw_owner=false;
    for (auto addr : asset_record->authority.owners)
    {
        issuer_account = _wimpl->_wallet_db.lookup_account(addr);
        if (issuer_account&&issuer_account->is_my_account)
        {
            try
            {
                _wimpl->withdraw_to_transaction(required_fees,
                    issuer_account->name,
                    trx,
                    required_signatures);

                get_withdraw_owner = true;
                break;
            }
    ...
    if (!get_withdraw_owner)
    {
        issuer_account = _wimpl->_wallet_db.lookup_account(asset_record->issuer_id);
        _wimpl->withdraw_to_transaction(required_fees,
            issuer_account->name,
            trx,
            required_signatures);
    ...
    optional_signatures_required = asset_record->authority.required;
    ...
    {
    case contact_data::contact_type_enum::account_name:
    {
        auto rec=_wimpl->self->get_account(recipient.data.as_string());
        to_addr = rec.owner_address();
        break;
    }
    case contact_data::contact_type_enum::public_key:
    {
        to_addr = address(public_key_type(recipient.data.as_string()));
        break;
    }
    case contact_data::contact_type_enum::address:
    {
        to_addr = address(recipient.data.as_string());
        break;
    }
    default:
        FC_CAPTURE_AND_THROW(unknown_account, (recipient));
    }
    trx.deposit_to_address(amount, to_addr);
    //const public_key_type recipient_key = _wimpl->deposit_from_transaction(trx, amount, *issuer_account, recipient, memo);
    ...
    auto rec=_wimpl->self->get_account_for_address(to_addr);
    if(rec.valid())
        entry.to_account = rec->owner_key;
    entry.amount = amount;
    entry.memo = memo;

    transaction_record.record_id = trx.id();
    transaction_record.ledger_entries.push_back(entry);
    transaction_record.fee = required_fees;
    transaction_record.trx = trx;
    transaction_record.memo = memo;

    return *this;
}

wallet_transaction_record mcc::wallet::transaction_builder::uia_update_permission_or_flag(const string & paying_account, const string & asset_symbol, const asset_record::flag_enum flag, const bool add_instead_of_remove, const bool update_authority_permission, const bool sign)
{
    const oasset_record asset_record = _wimpl->_blockchain->get_asset_record(asset_symbol);
    FC_ASSERT(asset_record.valid());
    ...
    const owallet_account_record payer_account = _wimpl->_wallet_db.lookup_account(paying_account);
