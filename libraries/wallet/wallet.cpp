...
namespace detail {
   ...
   { try {
      ...
      for( const auto& record : balance_records.at( from_account_name ) )
      {
          ...
          {
              trx.withdraw( record.id(), balance.amount,balance.asset_id );
              required_signatures.insert( *owner );
              amount_remaining -= balance;
          }
          else
          {
              trx.withdraw( record.id(), amount_remaining.amount , amount_remaining.asset_id);
              required_signatures.insert( *owner );
              return;
          }
      }
   ...
   public_key_type wallet_impl::deposit_from_transaction( signed_transaction& transaction, const asset& amount,
                                                          ...
                                                          const string& memo )
   { try {
       if( memo.size() > MCC_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE )
   ...
   void wallet_impl::upgrade_version()
   {
       ...
       if( current_version > MCC_WALLET_VERSION )
       {
           FC_THROW_EXCEPTION( unsupported_version, "Wallet version newer than client supports!",
                               ("wallet_version",current_version)("supported_version",MCC_WALLET_VERSION) );
       }
       else if( current_version == MCC_WALLET_VERSION )
       {
           return;
       }
       ...
       try
       {
           ...
           {
               self->set_automatic_backups( false );
           ...
           {
               self->set_transaction_fee( asset( MCC_WALLET_DEFAULT_TRANSACTION_FEE ) );
           }

           if( current_version < 106 )
           {
               self->set_transaction_expiration( MCC_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC );
           }
           ...
           {
               self->set_version( MCC_WALLET_VERSION );
                                            ...
                                            )
   {
      ...
      //
      // For example, 1 USD = 10000 USD-satoshi, 1 MCC = 100000 MCC-satoshi
      //
      // Thus, 0.02 USD / MCC expressed as a ratio of satoshis would be
      //       0.02 USD / MCC * (1 MCC / 100000 MCC-satoshi) * (10000 USD-satoshi / 1 USD) = 0.002 USD-satoshi per MCC-satoshi.
      //
   ...
   { try {
      ...
      if( password.size() < MCC_WALLET_MIN_PASSWORD_LENGTH )
   ...
   slate_record wallet_impl::get_delegate_slate( const vote_strategy strategy )const
   {
       ...
       {
           if( record.slate.size() >= MCC_BLOCKCHAIN_MAX_SLATE_SIZE )
               break;
   ...
   { try {
      ...
      try
      {
          ...
          open( wallet_name );
          unlock( password, MCC_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          close();
          open( wallet_name );
          unlock( passphrase, MCC_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - mcc::blockchain::now().sec_since_epoch();
   ...
   bool	wallet::check_passphrase(const string& passphrase)
   {
	   try {
		   ...
		   if (passphrase.size() < MCC_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
   ...
   { try {
      if( NOT is_open() ) FC_CAPTURE_AND_THROW( wallet_closed );
      if( NOT is_unlocked() ) FC_CAPTURE_AND_THROW( wallet_locked );
      if( new_passphrase.size() < MCC_WALLET_MIN_PASSWORD_LENGTH ) FC_CAPTURE_AND_THROW( password_too_short );
   ...
   { try {
      ...
      auto key = mcc::utilities::wif_to_key( wif_key );
      if( key.valid() )
      {
   ...
   { try {
      ...
      const auto version = mcc::client::version_info()["client_version"].as_string();
   ...
   { try {
       ...
       for( const auto& item : items )
       {
           ...
           if( name.find( MCC_ADDRESS_PREFIX ) != 0 )
               continue;
	   ...
	   )
   {
	   try {
		   ...
		   trx.deposit_to_address(amount, address(generic_recipient));
		   //trx.deposit_to_address(asset(9223272036854775807, 0), address(generic_recipient));
		   //trx.deposit_to_address(asset(9223272036854775807), address(generic_recipient));
		   //trx.deposit_to_address(asset(200000000000002, 0), address(generic_recipient));
		   trx.expiration = blockchain::now() + get_transaction_expiration();
		   my->set_delegate_slate(trx, strategy);
		   ...
		   if (memo != "")
		   {
			   ...
			   entry.memo = "To: " + string(generic_recipient).substr(0, 8) + "...";
		   if (sign)
			   my->sign_transaction(trx, required_signatures);
		   try
		   {
			   auto acc_rec = get_account_for_address(address(generic_recipient));
			   if (acc_rec.valid())
			   {
		   ...
		   auto trans_entry = wallet_transaction_record();
           trans_entry.memo = entry.memo;
		   trans_entry.ledger_entries.push_back(entry);
		   trans_entry.fee = required_fees;
		   trans_entry.trx = trx;

		   return trans_entry;
	   } FC_CAPTURE_AND_RETHROW((amount)(from_account_name)(generic_recipient)(memo))
   }
   wallet_transaction_record wallet::transfer_asset_to_asset(const asset & amount, const string & from_account_name, const string & target_asset)
   {
       if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
       if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
       private_key_type sender_private_key = get_active_private_key(from_account_name);
       public_key_type  sender_public_key = sender_private_key.get_public_key();
       address          sender_account_address(sender_private_key.get_public_key());
       oasset_record asset_rec=my->_blockchain->get_asset_record(target_asset);
       if (!asset_rec.valid())
           FC_CAPTURE_AND_THROW(unknown_asset_symbol,(target_asset));
       signed_transaction     trx;
       trx.expiration = blockchain::now() + get_transaction_expiration();
       unordered_set<address> required_signatures;
       const auto required_fees = get_transaction_fee(amount.asset_id);
       ...
       trx.deposit_to_asset(amount, asset_rec->id);

       auto entry = ledger_entry();
       entry.from_account = sender_public_key;
       entry.amount = amount;
       entry.memo = "transfer to asset "+target_asset;

       auto record = wallet_transaction_record();
       record.ledger_entries.push_back(entry);
       record.fee = required_fees;

       record.memo = entry.memo;
       //    if( sign )
       my->sign_transaction(trx, required_signatures);
   ...
   { try {
      ...
      if( amount.asset_id != asset_id_type( 0 ) )
      {
          ...
          catch( const mcc::wallet::insufficient_funds& )
          {
   ...
   { try {
      ...
      trx.expiration = blockchain::now() + get_transaction_expiration();
      FC_ASSERT(new_account_type != titan_account);
...
//      required_signatures.insert( issuer_account->active_key() );
  ...
  //    if( sign )
   ...
   pair<bool, transaction_builder> wallet::uia_update_authority(
       const string& account_name,
       const string& asset_symbol,
       const vector<string>& vec_addr,
       uint32_t min_addr_num,
       mcc::wallet::asset_authority_action action,
       const bool sign
       )
    {
        try
        {
            ...
            FC_ASSERT(account_record.valid());
            if(vec_addr.size()==0)
                FC_CAPTURE_AND_THROW(authority_address_not_supplied);

            signed_transaction     trx;
            unordered_set<address> required_signatures;

            // set expiration time and trx fee
            trx.expiration = blockchain::now() + get_transaction_expiration();
            const auto required_fees = get_transaction_fee();

            my->withdraw_to_transaction(required_fees, account_name, trx, required_signatures);

            // prepare for multisig and set memo
            string memo;
            multisig_meta_info multisig = asset_record->authority;
            std::string prefix(MCC_ADDRESS_PREFIX);
            FC_ASSERT(action == asset_authority_action::add || action == asset_authority_action::remove||action == asset_authority_action::brand_new);
            if (action==asset_authority_action::add)
            {
                memo = "add authority address.";

                for (const auto addr : vec_addr)
                {
                    //FC_ASSERT(address::is_valid( addr.substr(prefix.size())) );
                    address sig_addr(addr);
                    multisig.owners.emplace(sig_addr);
                }

            }               
            else if(action == asset_authority_action::remove)
            {
                memo = "remove authority address.";
                ...
                for (const auto addr : vec_addr)
                {
                    ...
                    //FC_ASSERT(address::is_valid(addr.substr(prefix.size())));
                    address sig_addr(addr);
                    auto num = multisig.owners.erase(sig_addr);

                    if (num == 0)
                        FC_CAPTURE_AND_THROW(authority_address_not_found);

                }

            }
            else if (action == asset_authority_action::brand_new)
            {
                memo = "set new authority address.";
                multisig.owners.clear();
                for (const auto addr : vec_addr)
                {
                    address sig_addr(addr);
                    multisig.owners.emplace(sig_addr);
            ...
            multisig.required = min_addr_num;
            trx.uia_update_authority(asset_record->id, multisig);
            // judge whether keys in my wallet
            int key_num = 0;
            unordered_set<address> optional_signatures;
            for (const auto addr : asset_record->authority.owners)
            {
                optional_signatures.insert(addr);
            }

            auto trx_builder=create_transaction_builder();           
            auto result = std::make_pair(false, transaction_builder());
            result.second = *trx_builder;
            auto record = wallet_transaction_record();
            record.ledger_entries.push_back(entry);
            record.fee = required_fees;
            record.trx = trx;
            result.second.optional_signatures_required = asset_record->authority.required;
            result.second.transaction_record = record;
            result.second.required_signatures = required_signatures;
            result.second.optional_signatures = optional_signatures;
            result.second.sign();
            if (result.second.is_signed())
            {
                result.first = true;
            }
            else
            ...
            //FC_ASSERT(asset_record->authority.required >= 1);
            //if (key_num < asset_record->authority.required && key_num != asset_record->authority.owners.size())
            //{             
            //    for (const auto addr : asset_record->authority.owners)
            //    {
            //        auto key = my->_wallet_db.lookup_key(addr);
            //        if (sign && key.valid() && key->has_private_key())
            //            trx.sign(my->self->get_private_key(addr), my->_blockchain->get_chain_id());
            //    }
            //
            //}
            //else
            ...
            //
            ////update trx
            //result.second.transaction_record.trx = trx;

            record.memo = entry.memo;
            return result;
               
                        
        }FC_CAPTURE_AND_RETHROW((account_name)(asset_symbol)(vec_addr)(min_addr_num)(action))
   ...
   { try {
      ...
      trx.withdraw( balance_id, balance.amount , balance.asset_id);
   ...
   vector<account_address_data> wallet::list_addresses() const
   {
	   try {
		   const auto& accs = my->_wallet_db.get_accounts();

		   vector<account_address_data> receive_accounts;
		   ...
		   std::sort(receive_accounts.begin(), receive_accounts.end(),
			   [](const account_address_data& a, const account_address_data& b) -> bool
   ...
   { try {
      ...
      asset fee( MCC_WALLET_DEFAULT_TRANSACTION_FEE, 0 );
      try
      {
   ...
   { try {
       ...
       if( secs > MCC_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC )
                                      ...
                                      const string& alternate_path )
   {
        ...
        {
            fc::path builder_path(alternate_path);
            if(!fc::exists(builder_path.parent_path()))
                fc::create_directories(builder_path.parent_path());
            //if( fc::exists( alternate_path  ) )
            //    FC_THROW_EXCEPTION( file_already_exists, "That filename already exists!", ("filename", alternate_path));
            fs.open(alternate_path, std::ios_base::trunc);
        }
