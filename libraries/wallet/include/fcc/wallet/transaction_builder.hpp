...
namespace mcc { namespace wallet {
   ...
   struct transaction_builder {
      ...
      bool required_signatures_signed = false;
      std::unordered_set<blockchain::address>                                      optional_signatures;

      std::unordered_set<blockchain::address>                                      signatures_unsigned;
      uint32_t     optional_signatures_required = 0;
      bool optional_signatures_enough = false;
      ...
      transaction_builder uia_issue_or_collect_fees(
      ...
      transaction_builder& uia_issue_to_asset(const asset& issue_asset,const asset_id_type& target_asset_id);
      void withdraw_transaction_fee(const string & account,const asset& amount);
      transaction_builder& operator=(transaction_builder&);
private:
...
} } //namespace mcc::wallet

FC_REFLECT_ENUM( mcc::wallet::vote_strategy,
        (vote_none)
        (vote_all)
        (vote_recommended)
        )
FC_REFLECT( mcc::wallet::transaction_builder, (transaction_record)(required_signatures)(optional_signatures)(optional_signatures_required)(optional_signatures_enough)(signatures_unsigned)(outstanding_balances) )
