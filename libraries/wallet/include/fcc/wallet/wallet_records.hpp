...
#include <fcc/blockchain/balance_record.hpp>
#include <fcc/blockchain/extended_address.hpp>
...
namespace mcc { namespace wallet {
   ...
   struct account_data : public mcc::blockchain::account_record
   {
       ...
       // Kept in MCC for upgrading old wallet versions; remove after 1.0
       int8_t           approved = 0;
   };
   struct account_address_data :public account_data
   {
	   account_address_data(){}
	   account_address_data(const account_data & data) :account_data(data)
	   {
...
FC_REFLECT_ENUM( mcc::wallet::wallet_record_type_enum,
...
FC_REFLECT_ENUM( mcc::wallet::property_enum,
        (version)
        (next_record_number)
        (next_child_key_index)
        (automatic_backups)
...
FC_REFLECT( mcc::wallet::wallet_property,
        (key)
        (value)
        )

FC_REFLECT_DERIVED( mcc::wallet::account_data, (mcc::blockchain::account_record),
		(is_my_account)
...
FC_REFLECT_DERIVED(mcc::wallet::account_address_data, (mcc::wallet::account_data),
	(owner_address)
	(active_address)
	)
FC_REFLECT( mcc::wallet::master_key,
        (encrypted_key)
        (checksum)
        )

FC_REFLECT( mcc::wallet::key_data,
        (account_address)
        (child_key_index)
        (public_key)
...
FC_REFLECT_ENUM( mcc::wallet::contact_data::contact_type_enum,
        (account_name)
        (public_key)
        (address)
        (btc_address)
...
FC_REFLECT( mcc::wallet::contact_data,
        (contact_type)
        (data)
        (label)
        (custom_data)
...
FC_REFLECT( mcc::wallet::approval_data,
        (name)
        (approval)
        (custom_data)
        )

FC_REFLECT( mcc::wallet::transaction_data,
        (record_id)
        (block_num)
        (is_virtual)
        (is_confirmed)
...
FC_REFLECT( mcc::wallet::ledger_entry,
        (from_account)
        (to_account)
        (amount)
        (memo)
...
FC_REFLECT( mcc::wallet::transaction_ledger_entry,
        (id)
        (block_num)
        (block_id)
        (timestamp)
...
FC_REFLECT_DERIVED( mcc::wallet::pretty_transaction_experimental, (mcc::wallet::transaction_ledger_entry),
        (inputs)
        (outputs)
        (balances)
        (notes)
...
FC_REFLECT( mcc::wallet::setting,
        (name)
        (value)
        )

FC_REFLECT( mcc::wallet::generic_wallet_record,
        (type)
        (data)
        )
...
namespace fc {
   ...
   template<typename T, mcc::wallet::wallet_record_type_enum N>
   struct get_typename< mcc::wallet::wallet_record<T,N> >
   {
   ...
   template<typename Type, mcc::wallet::wallet_record_type_enum N>
   struct reflector<mcc::wallet::wallet_record<Type,N>>
   {
      typedef mcc::wallet::wallet_record<Type,N>  type;
      ...
      {
          {
            typedef decltype(((mcc::wallet::base_record<N>*)nullptr)->wallet_record_index) member_type;
            visitor.TEMPLATE operator()<member_type,mcc::wallet::base_record<N>,&mcc::wallet::base_record<N>::wallet_record_index>( "index" );
          }
