...
namespace mcc { namespace wallet {
   ...
   class wallet
   {
      public:
		 ...
		 vector<account_address_data> list_addresses() const;
         ...
         wallet_transaction_record transfer_asset_to_asset(
             const asset& amount,
             const string& from_account_name,
             const string& target_asset
         );
         ...
         wallet_transaction_record register_account(
                 ...
                 mcc::blockchain::account_type new_account_type,
                 bool sign
                 );
         ...
         transaction_builder uia_issue_or_collect_fees_to_builder(
             const bool issue_new,
             const asset& amount,
             const string& generic_recipient,
             const string& memo,
         ...
         pair<bool, transaction_builder> uia_update_authority(
             const string& account_name,
             const string& asset_symbol,
             const vector<string>& vec_addr,
             uint32_t min_addr_num, 
             mcc::wallet::asset_authority_action add_or_remove,
             const bool sign
             );
...
FC_REFLECT_ENUM(mcc::wallet::asset_authority_action,
(add)
(remove)
(brand_new)
)