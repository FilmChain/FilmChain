...
{ try {
    ...
    if( okey_bid && okey_bid->has_private_key() )
    {
        ...
        printf("%s\n",fc::json::to_string(*bid_account_key).c_str());
...
{ try {
    ...
    for( const auto& op : transaction.operations )
    {
        switch( operation_type_enum( op.type ) )
        {
            ...
            case deposit_to_asset_op_type:
            {
                store_record |= scan_deposit_to_asset(op.as<deposit_to_asset_operation>(), *transaction_record, total_fee);
                has_deposit |= is_deposit;
                break;
            }
            default:
    ...
    /* Reconstruct fee */
    if( has_withdrawal && (!has_deposit||has_issue) )
    {
        for( auto& entry : transaction_record->ledger_entries )
        {
    ...
    for( const auto& op : transaction.operations )
    {
        switch( operation_type_enum( op.type ) )
        {
            ...
            case update_feed_op_type:
                ...
                break;
            case asset_update_permissions_op_type:
                store_record |= scan_asset_update_permissions(op.as<asset_update_permissions_operation>(), *transaction_record);
                break;
            default:
                break;
        }
...
bool mcc::wallet::detail::wallet_impl::scan_asset_update_permissions(const asset_update_permissions_operation & op, wallet_transaction_record & trx_rec)
{
    for (auto& entry : trx_rec.ledger_entries)
    {
        if (entry.from_account.valid())
        {
            ...
            if (opt_key_rec.valid() && opt_key_rec->has_private_key())
            {
                entry.memo = "update asset permissions  ";
                return true;
            }
        }
    }
...
bool mcc::wallet::detail::wallet_impl::scan_deposit_to_asset(const deposit_to_asset_operation & op, wallet_transaction_record & trx_rec, asset & total_fee)
{
    auto deposit_rec=_blockchain->get_deposit_to_asset_record(trx_rec.record_id);
    FC_ASSERT(deposit_rec.valid());
    asset_id_type asset_id = deposit_rec->deposit_asset_id;
    bool cache_deposit = false;
    for (auto deposit_it = deposit_rec->amounts.begin(); deposit_it != deposit_rec->amounts.end(); deposit_it++)
    {
        if(asset_id==0)
            total_fee -= asset(deposit_it->second, asset_id);
        obalance_record balance_rec=_blockchain->get_balance_record(deposit_it->first);
        FC_ASSERT(balance_rec.valid());
        deposit_operation de_op;
        de_op.condition = balance_rec->condition;
        de_op.amount = deposit_it->second;
        //cache_deposit |= scan_deposit(de_op, trx_rec, total_fee);
        if (balance_rec->condition.type != withdraw_signature_type)
            continue;
        const auto deposit = balance_rec->condition.as<withdraw_with_signature>();
        {
            ...
            if (okey_rec && okey_rec->has_private_key())
            {
                ...
                if (new_entry)
                {
                    ...
                    //entry.from_account = okey_rec->public_key;
                    entry.to_account = okey_rec->public_key;
                    entry.amount = asset(deposit_it->second, asset_id);
                    entry.memo = "deposit to asset";
                    trx_rec.ledger_entries.push_back(entry);
                }
            }
        }
...
{ try {
   ...
   if( !asset_symbol.empty() && asset_symbol != MCC_BLOCKCHAIN_SYMBOL )
   {
       try
       {
       ...
       if( !account_name.empty() )
       {
           ...
           for( const auto& entry : tx_record.ledger_entries )
           {
               if( entry.from_account.valid() )
               {
                   ...
                   if( account_record.valid() ) match |= (account_record->name == account_name)&&((entry.amount.asset_id== asset_id)|| (tx_record.fee.asset_id == asset_id));
                   if( match ) break;
               }
               if( entry.to_account.valid() )
               {
                   const auto account_record = get_account_for_address( *entry.to_account );
                   if( account_record.valid() ) match |= (account_record->name == account_name )&& (entry.amount.asset_id == asset_id);
                   if( match ) break;
               }
           }
           if( !match ) continue;
...
pretty_transaction wallet::to_pretty_trx( const wallet_transaction_record& trx_rec ) const
{
          ...
          pretty_entry.from_account = "MARKET";
		   ...
		   for (const auto & op : trx_rec.trx.operations)
		   {
			   ...
			   case withdraw_op_type:
			   {
				   ...
				   auto bal_rec = my->_blockchain->get_balance_record(withdraw_op.balance_id);
				   if (bal_rec.valid())
				   {
   ...
   vector<pretty_ledger_entry> temp_ledgers;
   auto it = pretty_trx.ledger_entries.begin();
   auto it_b = pretty_trx.ledger_entries.begin();
   temp_ledgers.push_back(*it);
   if ((it->memo.substr(0, 3)) == "ASK")
   {
       temp_ledgers.back().to_account = "MARKET";
   }
   it++;
   for (; it != pretty_trx.ledger_entries.end(); it++,it_b++)
   {
       if (it->to_account == "MARKET" && (it_b->from_account == it_b->to_account)&&(it_b->amount == it_b->amount))
       {
           temp_ledgers.back().to_account = "MARKET";
       }
       else
   ...
   pretty_trx.ledger_entries = temp_ledgers;
   ...
   string start = trx_rec.memo;

   
   if (pretty_trx.ledger_entries.size() > 0)
   {
       string end = pretty_trx.ledger_entries[0].memo;
       if (pretty_trx.ledger_entries[0].memo.length() == 0)
       {
           pretty_trx.ledger_entries[0].memo = trx_rec.memo;
       }
       else if (trx_rec.memo.length() > 0)
       {
           pretty_trx.ledger_entries[0].memo = trx_rec.memo + ";" + pretty_trx.ledger_entries[0].memo;
       }   
   }
