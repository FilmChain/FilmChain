...
namespace mcc { namespace tscript {
...
public:
    ...
    mcc::blockchain::genesis_state genesis_config;
    mcc::net::simulated_network_ptr sim_network;
};
...
void client_context::configure_client_from_args()
{
    ...
    std::string arg_0 = "mccshares_client";
...
context::context()
{
    this->sim_network = std::make_shared<mcc::net::simulated_network>("tscript");
...
void context::set_genesis(const fc::path& genesis_json_filename)
{
    ...
    this->genesis_config = fc::json::from_file(genesis_json_filename).as<mcc::blockchain::genesis_state>();
...
void context::create_client(const std::string& name)
{
    ...
    cc->client = std::make_shared<mcc::client::client>("tscript", this->sim_network);
...
void interpreter::cmd_x(const fc::variants& cmd)
{
    ...
    std::vector<mcc::tscript::client_context_ptr> targets;
    
    this->ctx->get_clients_by_variant( cmd[1], targets );

    std::cout << "targets found: " << targets.size() << "\n";

    for( mcc::tscript::client_context_ptr& t : targets )
    {
    ...
    )
{
    ...
    mcc::tscript::interpreter interp;
...
{
    for( int i=1; i<argc; i++ )
        mcc::tscript::run_tscripts( "tmp/genesis.json", argv[i] );
    return 0;
}
