...
# https://github.com/BitShares/mccshares/issues/880
substitutions = dict([
...
with open("dns-dev-fund.json") as devfund:
    ...
    with open("dns-nov-5.json") as dns:
        ...
        scale *= 10 ** 8
        print "MCC per DNS normal: " + str(scale / 1000)
        ...
        mcc_to_bonus = int(scale * pangel_dns) 

print "mcc to bonus: " + str(mcc_to_bonus / (10**8))
...
with open("dns-collapse.json") as collapse:
    ...
    mcc_for_exchanges = mcc_to_bonus * 2/3 # 10000000 * 10**8
    print "mcc for exchanges: " + str(mcc_for_exchanges / 10**8)
    print "mcc for on-chain: " + str((mcc_to_bonus - mcc_for_exchanges) / 10**8)
    with open("exchanges.json") as exchanges:
        ...
        for item in exchanges:
            print item["name"]
            mcc_to_exchange = int(mcc_for_exchanges * 1.0 * item["proportion"] / ex_total)
            exodus_balances.append([item["address"], mcc_to_exchange])
            mcc_added += mcc_to_exchange
            print mcc_to_exchange

    scale = 1.0 * (mcc_to_bonus - mcc_for_exchanges) / collapse_total
    print "MCC per DNS for normal claimed balances " + str(scale / 1000)
...
# mcc_added and mcc_excluded add up at this point. Any further changes have to adjust both balances
...
print "mcc_added: " + str(mcc_added)
print "mcc_excluded: " + str(mcc_excluded)
print "total reviewed: " + str(mcc_added + mcc_excluded)

with open("mcc-sharedrop.json", "w") as sharedrop:
    sharedrop.write(json.dumps(output, indent=4))


# coinbase substitutions - signatures confirmed by toast
# https://mccsharestalk.org/index.php?topic=9516.msg123741#msg123741
...
#  more:  https://github.com/bitsuperlab/mccshares_play/blob/develop/libraries/blockchain/pts_ags_snapshot/substitution.txt
