    ...
    load_recent_markets: ->
        ...
        @wallet_api.get_setting("recent_markets").then (result) =>
            if result and result.value
                ...
                @recent_markets.push r for r in JSON.parse(result.value.replace(/MCCX/g,'MCC').replace(/GLD/g,'GOLD'))
        ...
        @blockchain_api.market_price_history(market.asset_base_symbol, market.asset_quantity_symbol, start_time, 10 * interval * desired_nr_items, fetch_interval).then (result) =>
            ...
            for t, index in result by -1
                ...
                if time < interval_time 
                    mccc_data.unshift [interval_time, interval_o, interval_h, interval_l, interval_c]
                    if inverted
            ...
            # console.log "items in final array:", mccc_data.length
            # console.log "first date:", new Date(mccc_data[0][0]), "last date:",new Date(mccc_data[mccc_data.length - 1][0])
            if market.orig_market and inverted
                market.orig_market.price_history = mccc_data
                market.orig_market.volume_history = volume_data
            else
                market.price_history = mccc_data
