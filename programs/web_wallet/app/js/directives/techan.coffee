initChart = (scope, svg) ->
    ...
    dim.indicator.top = dim.mccc.height+dim.indicator.padding
    ...
    draw = () ->
        ...
        svg.select("g.mccc .axis").call(yAxis)
        ...
        svg.select("g.crosshair.mccc").call(mcccCrosshair.refresh)
    ...
    volume = techan.plot.volume()
            .accessor(candlestick.accessor())   # Set the accessor to a mccc accessor so we get highlighted bars
            .xScale(x)
            .yScale(yVolume)
    ...
    mcccAnnotation = techan.plot.axisannotation()
            .axis(yAxis)
    ...
    mcccCrosshair = techan.plot.crosshair()
            .xScale(timeAnnotation.axis().scale())
            .yScale(mcccAnnotation.axis().scale())
            .xAnnotation(timeAnnotation)
            .yAnnotation([mcccAnnotation, percentAnnotation, volumeAnnotation])
    ...
    mcccSelection = svg.append("g")
            .attr("class", "mccc")
            .attr("transform", "translate(0,0)")

    mcccSelection.append("g")
            .attr("class", "axis")
    ...
    y.domain(techan.scale.plot.mccc(data.slice(indicatorPreRoll)).domain())
    ...
    svg.select("g.crosshair.mccc").call(mcccCrosshair).call(zoom)
