angular.module("app").controller "BrainWalletController", ($scope, $rootScope, $rootElement, $modal, $log, $location, $idle, $q, $timeout, $http, RpcService, Wallet, Growl) ->
    ...
    return unless window.mcc
    
    $idle.unwatch()
    $scope.stopIdleWatch()
    
    $rootScope.splashpage = true
    $scope.has_secure_random = mcc.wallet.Wallet.has_secure_random()
    ...
    dictionary=(url = '/dictionary_en.txt')->
        ...
        $http.get(url).success (data)->
            ...
            dictionary_hash = mcc.hash.sha1 data
    ...
    generateBrainkey=(
        ...
        entropy += mcc.secureRandom.randomBuffer(32).toString()
        #console.log '... entropy',entropy
        
        randomBuffer = mcc.hash.sha512 entropy # 64 bytes
    ...
    on_mouse_event = (event) ->
        ...
        if private_entropy.length >= 1000
            ...
            mcc.wallet.Wallet.add_entropy private_entropy.join ''
            private_entropy.length = 0
            return
        
        num = (x, y) ->
            new Date().getTime() + Math.pow(x,5) + Math.pow(y,4) +
            mcc.secureRandom.randomUint8Array(1)[0]
