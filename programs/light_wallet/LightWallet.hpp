...
#include <mcc/light_wallet/light_wallet.hpp>
...
public:
   ...
   {
      return mcc::blockchain::chain_database::is_valid_account_name(convert(name));
   }
   Q_INVOKABLE bool isValidKey(QString key)
   {
      try {
         return mcc::blockchain::public_key_type(convert(key)) != mcc::blockchain::public_key_type();
      } catch (...) {
         return false;
      }
   }
   ...
   {
      return MCC_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE;
   }
   QString baseAssetSymbol() const
   {
      return QStringLiteral(MCC_BLOCKCHAIN_SYMBOL);
   }
   QString keyPrefix() const
   {
      return QStringLiteral(MCC_ADDRESS_PREFIX);
   }
...
private:
   ...
   fc::path m_walletPath = fc::path(m_dataDir.toStdWString())
         / "wallet_data.json";
   mcc::light_wallet::light_wallet m_wallet;
   ...
   void updateAccount(const mcc::light_wallet::account_record& account);
};
