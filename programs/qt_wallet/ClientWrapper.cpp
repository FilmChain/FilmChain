...
#include <mcc/net/upnp.hpp>
#include <mcc/net/config.hpp>
#include <mcc/db/exception.hpp>
...
{
  _init_complete.wait();
  QSettings("MCCShares", MCC_BLOCKCHAIN_NAME).setValue("crash_state", "no_crash");
  if (_client)
     _mccshares_thread.async([this]{
       _client->stop();
       /*
...
{
  QString data_dir = QString::fromStdWString(mcc::client::get_data_dir(boost::program_options::variables_map()).generic_wstring());
...
void ClientWrapper::initialize(INotifier* notifier)
{
  ...
  _init_complete = _mccshares_thread.async( [=](){
    try
    {
      main_thread->async( [&]{ Q_EMIT status_update(tr("Starting %1").arg(qApp->applicationName())); });
      _client = std::make_shared<mcc::client::client>("qt_wallet");
      ...
      const mcc::client::config& loadedCfg = _client->configure( data_dir.toStdWString() );
      ...
      if( upnp )
      {
        _upnp_service = std::make_shared<mcc::net::upnp_service>();
...
{
  fc::variant_object result = _mccshares_thread.async( [this](){ return _client->get_info(); }).wait();
...
{
  QSettings ("MCCShares", MCC_BLOCKCHAIN_NAME).setValue("data_dir", data_dir);
}
