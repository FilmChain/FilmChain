...
public:
   ...
   virtual void on_config_loaded(const mcc::client::config& config) override
   {
...
void installCrashRptHandler(const char* appName, const char* appVersion)
{
   ...
   fc::variant_object version_info(mcc::client::version_info());
...
MCCSharesApp* MCCSharesApp::_instance = nullptr;

#define APP_NAME "MCCShares"

static std::string CreateMCCSharesVersionNumberString()
{
   return mcc::client::version_info()["client_version"].as_string();
}

MCCSharesApp::MCCSharesApp(int& argc, char** argv)
...
MCCSharesApp::~MCCSharesApp()
{
   assert(_instance == this && "Only one instance allowed at time");
   _instance = nullptr;
}

int MCCSharesApp::run(int& argc, char** argv)
{
   installCrashRptHandler(APP_NAME, CreateMCCSharesVersionNumberString().c_str());

   MCCSharesApp app(argc, argv);
   QTranslator mccsharesTranslator;
   mccsharesTranslator.load(QLocale::system().name()+".qm");
   app.installTranslator(&mccsharesTranslator);
...
int MCCSharesApp::run()
{
   setOrganizationName("MCCShares");
   setOrganizationDomain("mccshares.org");
   setApplicationName(MCC_BLOCKCHAIN_NAME);
   ...
   sock->connectToServer(MCC_BLOCKCHAIN_NAME);
...
void MCCSharesApp::prepareStartupSequence(ClientWrapper* client, Html5Viewer* viewer, MainWindow* mainWindow, QSplashScreen* splash)
{
   viewer->connect(viewer->webView(), &QGraphicsWebView::urlChanged, [viewer,client,mainWindow] (const QUrl& newUrl) {
      ...
      viewer->webView()->page()->mainFrame()->addToJavaScriptWindowObject("mccshares", client);
...
QLocalServer* MCCSharesApp::startSingleInstanceServer(MainWindow* mainWindow)
{
   QLocalServer* singleInstanceServer = new QLocalServer();
   if (!singleInstanceServer->listen(MCC_BLOCKCHAIN_NAME))
   {
      std::cerr << "Could not start new instance listener. Attempting to remove defunct listener... ";
      QLocalServer::removeServer(MCC_BLOCKCHAIN_NAME);
      if (!singleInstanceServer->listen(MCC_BLOCKCHAIN_NAME))
      {
...
bool MCCSharesApp::notify(QObject* receiver, QEvent* e)
{
   APP_TRY
   {
...
void MCCSharesApp::onExceptionCaught(const fc::exception& e)
{
   displayFailureInfo(e.to_detail_string());
}

void MCCSharesApp::onUnknownExceptionCaught()
{
   std::string detail("Unknown exception caught");
   displayFailureInfo(detail);
}

void MCCSharesApp::displayFailureInfo(const std::string& detail)
{
   elog("${e}", ("e", detail));
