...
//    $LOGIN_PAGE_DOMAIN_AND_PATH = "Set this if login landing page != this page";
    ...
    $mccshares = new Bitcoin($RPC_SERVER_USER, $RPC_SERVER_PASS, $RPC_SERVER_ADDRESS, $RPC_SERVER_PORT, $RPC_SERVER_PATH);

    $mccshares->open($RPC_SERVER_WALLET);
    $mccshares->unlock(2, $RPC_SERVER_WALLET_PASS);   
    
    if(isset($_REQUEST["client_key"]))
    {
        $loginPackage = $mccshares->wallet_login_finish($_REQUEST["server_key"], $_REQUEST["client_key"], $_REQUEST["signed_secret"]);
        ...
        $userAccount = $mccshares->blockchain_get_account($loginPackage["user_account_key"]);
        ...
        $url = $mccshares->wallet_login_start($BITSHARES_USER_NAME);
        $url .= $absolute_url;
        ?>
